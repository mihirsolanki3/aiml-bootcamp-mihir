...
class AsyncSoftReadWriteLock:
    ...
    async def acquire_read(
        ...
        await self._run_acquire(
            functools.partial(self._lock.acquire_read, timeout, blocking=blocking)
        )
    ...
    async def acquire_write(
        ...
        self._raise_if_inherited()
        await self._run_acquire(
            functools.partial(self._lock.acquire_write, timeout, blocking=blocking)
        )
        return AsyncAcquireSoftReadWriteReturnProxy(lock=self)
    ...
    def _submit(
        ...
        return loop.run_in_executor(
            self._executor, _capture_call, functools.partial(func, *args, **kwargs)
        )
