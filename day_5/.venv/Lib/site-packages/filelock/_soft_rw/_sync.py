...
_SUPPORTS_DIR_FD: Final[bool] = (
    sys.platform != "win32" and os.open in os.supports_dir_fd
)

_ALL_INSTANCES: Final[WeakValueDictionary[int, SoftReadWriteLock]] = (
    WeakValueDictionary()
)
...
class SoftReadWriteLock(metaclass=_SoftRWMeta):
        ...
        *,
        ...
        if mode == "write":
            marker_name, is_reader = self._acquire_writer_slot(
                token, deadline=deadline, blocking=blocking
            )
        else:
        ...
        with self._locks.internal:
            ...
            try:
                ...
                BaseException
            ) as error:  # ruff:ignore[blind-except]  # clear the slot below and re-raise
                self._hold = None
                start_error = error
    ...
    def _validate_reentrant(self, mode: _Mode) -> AcquireReturnProxy:
        ...
        assert (
            hold is not None
        )  # ruff:ignore[assert]  # callers dispatch here only inside the self._hold is not None branch
        if hold.mode != mode:
        ...
        *,
        ...
        def readers_drained_touching() -> bool:
            with self._locks.state:
                ...
                if not self._touch_writer_marker_if_ours(
                    token
                ) and not self._claim_writer_marker(token):
                    return False
        ...
        try:
            self._wait_for(
                readers_drained_touching, deadline=deadline, blocking=blocking
            )
        except Timeout:
    ...
    def _touch_writer_marker_if_ours(self, token: str) -> bool:
        ...
        try:
            try:
                ...
                OSError
            ):  # pragma: no cover - e.g. EAGAIN from a hostile FIFO that has a writer attached
                return False
    ...
    def _open_readers_dir(self) -> None:
        ...
        if self._readers_dir_fd is None and _SUPPORTS_DIR_FD:  # pragma: needs dir-fd
            with _fork_transition():
                fd = os.open(
                    self._paths.readers,
                    os.O_RDONLY | getattr(os, "O_DIRECTORY", 0) | _O_NOFOLLOW,
                )
                try:
                    ...
                    try:
                        ...
                        BaseException
                    ) as close_error:  # ruff:ignore[blind-except]  # both errors surface via the group below
                        _raise_grouped_errors(
        ...
        try:
            with closing(self._iter_reader_entries()) as entries:
                for name, dirfd_relative in entries:
                    names.append(
                        (name, self._readers_dir_fd if dirfd_relative else None)
                    )
        except OSError:  # pragma: no cover - transient NFS scandir hiccup
            return
        for name, fd in names:
            _break_stale_marker(
                name, stale_threshold=self.stale_threshold, now=now, dir_fd=fd
            )
    ...
    def _refresh_marker(self) -> bool:
        ...
        try:
            try:
                ...
                OSError
            ):  # a transient read error or EAGAIN from a hostile FIFO; retry rather than drop the lock
                return True
            info = _parse_marker_bytes(data)
...
def _read_marker(
    ...
    try:
        ...
        OSError
    ):  # pragma: no cover - marker vanished or turned unreadable between open and read
        return None
    finally:
        os.close(fd)
...
def _break_stale_marker(  # ruff:ignore[too-many-return-statements]  # each return is a distinct abort/commit point in the break protocol
    ...
    *,
    ...
    try:
        ...
        OSError
    ):  # pragma: no cover - race where the marker vanishes between read and rename
        return False

    read_after = _read_marker(break_name, dir_fd=dir_fd)
    if (
        read_after is None
    ):  # pragma: no cover - race where a peer unlinks the break-name file
        return False
    info_after, mtime_after = read_after
    if (
        info_after is None
    ):  # pragma: no cover - content replaced post-rename by a racing peer
        _unlink(break_name, dir_fd=dir_fd)
        return True
    if not hmac.compare_digest(
        info_before.token, info_after.token
    ):  # pragma: no cover - race only
        return False
...
def _cleanup_all_instances() -> (
    None
):  # pragma: no cover - runs from atexit at interpreter shutdown
