...
def _capture_call(func: Callable[[], _T]) -> _BackendOutcome[_T]:
    try:
        ...
        BaseException
    ) as error:  # ruff:ignore[blind-except]  # backend control-flow exceptions are operation results
        return _BackendOutcome(error=error)


def _raise_cancelled_error(
...
async def _capture_awaitable(awaitable: Awaitable[_T]) -> _BackendOutcome[_T]:
    try:
        ...
        BaseException
    ) as error:  # ruff:ignore[blind-except]  # backend cancellation must remain distinct from caller cancellation
        return _BackendOutcome(error=error)
