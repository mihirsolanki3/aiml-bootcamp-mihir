...
class SoftFileLease(MarkerSoftFileLock):
    ...
    def __init__(
        ...
        **kwargs: Unpack[LockOptions],
        ...
        self._claims: _LeaseClaimHolder = (
            _ThreadLocalLeaseClaimHolder
            if self.is_thread_local()
            else _LeaseClaimHolder
        )()
    ...
    def _published_record(self) -> OwnerRecord:
        return (
            super()
            ._published_record()
            ._replace(token=self._claim.token, lease_duration=self._lease_duration)
        )
    ...
    def _stop_heartbeat(self) -> None:
        ...
        if (
            heartbeat.thread.ident is not None
            and heartbeat.thread is not current_thread()
        ):
    ...
    def _refresh_until_stopped(
        ...
        while not stop.wait(self._heartbeat_interval):
            ...
            if outcome == "ok":
                ...
                >= self._lease_duration - self._heartbeat_interval
            ):
    ...
    def _report_compromise(
        ...
        claim.compromise = LeaseCompromise(
            lock_file=self.lock_file, token=token, reason=reason, error=error
        )
