...
if sys.platform == "win32":  # pragma: win32 cover
    ...
    class UnixFileLock(BaseFileLock):
        ...
        def _switch_to_soft_lock(self, fd: int, missing_flock: OSError) -> None:
            ...
            os.close(fd)
            if (
                not self._fallback_to_soft
                or self._preserve_lock_file
                or self._on_acquired is not None
            ):
        ...
        def _finalize_locked_fd(self, fd: int) -> None:
            ...
            try:
                ...
                raise
            if keep:
                self._mark_descriptor_owned(
                    fd, (stat_result.st_dev, stat_result.st_ino)
                )
            else:
                self._mark_descriptor_released()
                os.close(fd)
