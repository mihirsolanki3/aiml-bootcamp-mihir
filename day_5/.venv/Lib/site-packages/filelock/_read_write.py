    ...
    def functions(
        ...
        with self._lock:  # pragma: <3.12 cover  # pragma: needs fork
            if self._functions is None:
                ...
                increment_address = ctypes.cast(
                    ctypes.pythonapi.Py_IncRef, ctypes.c_void_p
                ).value
                decrement_address = ctypes.cast(
                    ctypes.pythonapi.Py_DecRef, ctypes.c_void_p
                ).value
                if (
                    increment_address is None or decrement_address is None
                ):  # pragma: no cover - resolved CPython API
                    msg = "CPython reference functions have no address"
                    raise RuntimeError(msg)
                self._functions = (
                    cast(
                        "Callable[[sqlite3.Connection], None]",
                        function_type(increment_address),
                    ),
                    cast(
                        "Callable[[sqlite3.Connection], None]",
                        function_type(decrement_address),
                    ),
                )
...
class _ForkedDatabaseRegistry:
    ...
    def raise_if_poisoned(self, path: pathlib.Path) -> None:
        ...
        with self._lock:
            ...
            poisoned = (
                all_paths_poisoned
                or path in self._paths
                or (identity is not None and identity in self._identities)
            )
    ...
    def _reset_after_fork_in_child(self) -> None:  # pragma: forked child
        self._lock = threading.RLock()
        self._all_paths_poisoned = self._all_paths_poisoned or (
            _IS_PYPY and self._sqlite_used
        )
        self._sqlite_used = False
    ...
    def close(self) -> None:
        with _sqlite_transition():
            ...
            with _fork_transition():
                ...
                if (
                    decrement := self._decrement_escrow
                ) is not None:  # pragma: <3.12 cover  # pragma: needs fork
        ...
        *,
        ...
        with cls._instances_lock:
            if normalized not in cls._instances:
                if (
                    normalized in cls._instances_under_construction
                ):  # pragma: no cover - exercised in an audit callback
...
class ReadWriteLock(metaclass=_ReadWriteLockMeta):
    ...
    def __del__(self) -> None:
        if (
            _GETPID() == getattr(self, "_creator_pid", None)
            and (connection := getattr(self, "_con", None)) is not None
        ):
            with suppress(sqlite3.Error, RuntimeError):
                connection.close()
    ...
    def _reset_after_fork_in_child(self) -> None:  # pragma: forked child
        if self._con is not None:
            _FORKED_DATABASES.poison_after_fork(
                self._canonical_path, self._connection_identity
            )
        self._con = None
        self._connection_transaction_released = True
    ...
    def _acquire(
        ...
        with _fork_transition():
            ...
            with self._internal_lock:
                ...
                if (
                    thread_id in self._acquisition_thread_ids
                ):  # pragma: no cover - exercised in an audit callback
    ...
    def _do_acquire_inner(
        ...
        *,
        ...
        try:
            ...
            ) and "database is locked" in str(error):
    ...
    def _open_for_acquisition(
        ...
        with _sqlite_transition():
            ...
            (
                self._con,
                self._connection_transaction_released,
        ...
        *,
        ...
        with _sqlite_transition():
            ...
            connection.executescript(
                f"PRAGMA busy_timeout={timeout_ms}; PRAGMA journal_mode=MEMORY;"
            ).close()
            ...
            self._raise_if_process_changed(operation_pid)
            statements = (
                f"PRAGMA busy_timeout={recomputed}; "
                if recomputed != timeout_ms
                else ""
            )
            statements += (
    ...
    def _raise_if_acquiring(
        self, operation: Literal["acquire", "close", "release"]
    ) -> None:
        with self._internal_lock:
            active_in_current_thread = (
                threading.get_ident() in self._acquisition_thread_ids
            )
        if (
            active_in_current_thread
        ):  # pragma: no cover - exercised in an audit callback
...
def _abort_forked_sqlite_transition() -> None:  # pragma: forked child
    if _SQLITE_TRANSITION_CONTEXT.depth:
        os._exit(
            _UNSAFE_FORK_EXIT_STATUS
        )  # inherited SQLite handles cannot be used or closed safely
...
def timeout_for_sqlite(timeout: float, *, blocking: bool, already_waited: float) -> int:
    ...
    timeout_ms = int(
        (max(timeout - already_waited, 0) if timeout > 0 else timeout) * 1000
    )
