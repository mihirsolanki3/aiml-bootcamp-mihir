...
if sys.platform == "win32":  # pragma: win32 cover
    ...
    def process_start_token(pid: int) -> int | None:
        ...
        handle = _KERNEL32.OpenProcess(
            _WIN_PROCESS_QUERY_LIMITED_INFORMATION, _WIN_INHERIT_HANDLE, pid
        )
        if not handle:
            return None
    ...
    if sys.platform in {"linux", "android"}:  # pragma: linux cover
        ...
        def _read_boot_id() -> int:
            ...
            try:
                ...
                OSError,
                ValueError,
            ):  # pragma: no cover  # the kernel always exposes boot_id as a UUID on Linux
                return 0
        ...
        def process_start_token(pid: int) -> int | None:
            ...
            if (
                len(fields) <= _STARTTIME_INDEX
            ):  # pragma: no cover  # a truncated /proc read, never seen in practice
                return None
            try:
                starttime = int(fields[_STARTTIME_INDEX])
            except (
                ValueError
            ):  # pragma: no cover  # /proc always renders starttime as an integer
                return None
        ...
        def process_start_token(pid: int) -> int | None:
            ...
            if (
                _LIBC.sysctl(mib, 4, buffer, ctypes.byref(length), None, 0) != 0
                or length.value < _TIMEVAL_SIZE
            ):
                return None
