...
if sys.platform == "win32":  # pragma: win32 cover
    ...
    _FILE_OPEN_IF: Final[int] = (
        3  # open the file if it exists, create it otherwise; the NtCreateFile OPEN_ALWAYS
    )
    ...
    _CREATE_OPTIONS: Final[int] = (
        _FILE_SYNCHRONOUS_IO_NONALERT
        | _FILE_NON_DIRECTORY_FILE
        | _FILE_OPEN_REPARSE_POINT
    )
    _OBJ_CASE_INSENSITIVE: Final[int] = (
        0x00000040  # Win32 name lookups are case-insensitive
    )
    ...
    class _UNICODE_STRING(
        ctypes.Structure
    ):  # ruff:ignore[invalid-class-name]  # mirrors the Win32 struct name
        _fields_ = (
    ...
    class _IO_STATUS_BLOCK(
        ...
        _fields_ = (
            (
                "Status",
                ctypes.c_void_p,
            ),  # a union of NTSTATUS and PVOID, so it is pointer-sized
    ...
    class _OVERLAPPED(ctypes.Structure):  # mirrors the Win32 struct name
        _fields_ = (
            (
                "Internal",
                ctypes.c_void_p,
            ),  # ULONG_PTR: pointer-sized, not DWORD, or the x64 layout corrupts Offset
            ("InternalHigh", ctypes.c_void_p),
            (
                "Offset",
                wintypes.DWORD,
            ),  # the DUMMYUNIONNAME struct, flattened: low 32 bits of the byte offset
    ...
    def _lock_fd_nonblocking(fd: int) -> bool:
        ...
        overlapped = (
            _OVERLAPPED()
        )  # zero-initialized, so Offset/OffsetHigh/hEvent are 0
    ...
    def _unlock_fd(fd: int) -> None:
        overlapped = (
            _OVERLAPPED()
        )  # the same offset 0 and one-byte length the lock used
        # Unlocking the exact range we hold does not fail.
        if not _kernel32.UnlockFileEx(
            msvcrt.get_osfhandle(fd), 0, 1, 0, ctypes.byref(overlapped)
        ):  # pragma: no cover
    ...
    class WindowsFileLock(BaseFileLock):
        ...
        def _acquire(self) -> None:
            ...
            try:
                ...
                BaseException
            ):  # pragma: no cover  # cleanup only if the lock attempt itself raises
                os.close(fd)
                raise
            if not locked:
                os.close(
                    fd
                )  # another holder owns the byte-range lock; let the retry loop try again
        ...
        def _release(self) -> None:
            ...
            if (
                not self._preserve_lock_file
            ):  # preserve_lock_file keeps a stable file identity for the caller (#605)
    ...
    def _open_non_reparse_fd(path: str, mode: int) -> int | None:
        ...
        try:
            ...
            BaseException
        ):  # pragma: no cover  # open_osfhandle audits too; a hook raising must not leak the handle
            _kernel32.CloseHandle(handle)
            raise
    ...
    def _nt_open(path: str, *, read_only: bool) -> tuple[int, int]:
        ...
        status = (
            _ntdll.RtlDosPathNameToNtPathName_U_WithStatus(
                path, ctypes.byref(nt_name), None, None
            )
            & 0xFFFFFFFF
        )
        if status != _STATUS_SUCCESS:
