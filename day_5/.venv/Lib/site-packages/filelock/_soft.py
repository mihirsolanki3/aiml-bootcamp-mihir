...
class SoftFileLock(BaseFileLock):
        ...
        try:
            ...
            with suppress(OSError):
                if (
                    ...
                    and _file_identity(os.lstat(self.lock_file)) == identity
                ):
    ...
    def _release(self) -> None:
        ...
        assert (
            fd is not None
        )  # ruff:ignore[assert]  # _release runs only while held, so the descriptor is set
        ...
        # Marker cleanup must also run for control-flow exceptions, and both failures must remain observable.
            ...
            try:
                ...
                BaseException
            ) as cleanup_error:  # ruff:ignore[blind-except]  # preserve control-flow cleanup failures
    ...
    def _windows_unlink_if_ours(
        ...
        for attempt in range(_UNLINK_MAX_RETRIES):
            ...
            try:
                ...
                OSError
            ) as exc:  # ruff:ignore[try-except-in-loop]  # each attempt's errno drives the retry choice
...
def _read_lock_file(path: str) -> tuple[str | None, float, int]:
    ...
    try:
        ...
        if not stat.S_ISREG(
            st.st_mode
        ):  # pragma: no cover  # only a non-regular node swapped in after the lstat
            return None, st.st_mtime, st.st_ino
...
def _parse_lock_holder(content: str | None) -> tuple[int, str, int | None] | None:
    ...
    try:
        pid = int(lines[0])
        start_token = (
            int(lines[2]) if len(lines) == _MARKER_WITH_START_TOKEN_LINE_COUNT else None
        )
    except ValueError:
        return None
