...
_ASYNC_RELEASE_CANCELLATION_ERRORS: Final[str] = (
    "lock release cancellation and backend release both failed"
)
_ASYNC_CONTEXT_RELEASE_ERRORS: Final[str] = (
    "context body, release cancellation, and backend release failed"
)
_ASYNC_RELEASE_CANCELLATION_MARKER_ATTR: Final[str] = (
    "_filelock_async_release_cancellation"
)
...
class AsyncFileLockMeta(FileLockMeta):
    def __call__(  # ruff:ignore[too-many-arguments]  # forwards the public constructor's documented parameters
        cls: type[
            _AT
        ],  # ruff:ignore[invalid-first-argument-name-for-method]  # metaclass __call__ receives the class being constructed
...
class BaseAsyncFileLock(BaseFileLock, metaclass=AsyncFileLockMeta):
    ...
    async def _run_acquire_attempt(self) -> None:
        ...
        try:
            ...
            try:
                ...
                BaseException
            ) as error:  # ruff:ignore[blind-except]  # reported with the cancellation below
                acquire_error = error
            ...
            if acquire_error is not None:
                ...
                self._raise_cancelled_errors(
                    "lock acquisition cancellation and backend attempt both failed",
                    cancellation,
                    acquire_error,
                )
            if rollback_error is not None:
                self._raise_cancelled_errors(
                    "lock acquisition cancellation and rollback both failed",
                    cancellation,
                    rollback_error,
                )
            raise
    ...
    async def _rollback_backend_cancelled_acquire(
    ...
    def _raise_cancelled_errors(
        ...
        if self._context_error_policy == "group":
            marker = (
                (
                    _ASYNC_RELEASE_CANCELLATION_MARKER_ATTR,
                    _ASYNC_RELEASE_CANCELLATION_MARKER,
                )
            ...
            _raise_grouped_errors(
                message, cancellation, first_error, second_error, marker=marker
            )
        if (
            context := first_error.__context__
        ) is not None and context is not cancellation:
    ...
    async def release(
        ...
        None
    ):  # ty: ignore[invalid-method-override]  # ruff:ignore[boolean-type-hint-positional-argument, boolean-default-value-positional-argument]  # public API: positional bool kept for backwards compatibility
        """
    ...
    async def _release_serialized(self, *, force: bool) -> None:
        ...
        try:
            ...
            try:
                ...
                BaseException
            ) as release_error:  # ruff:ignore[blind-except]  # cancellation and backend failure must both surface
                self._commit_release_if_released()
                self._raise_cancelled_errors(
                    _ASYNC_RELEASE_CANCELLATION_ERRORS, cancellation, release_error
                )
        ...
        try:
            ...
            BaseException
        ):  # state follows the backend for control-flow exceptions too
            self._commit_release_if_released()
            raise
        self._commit_release()
    ...
    def _start_internal_method(
        ...
        if iscoroutinefunction(method):
            return asyncio.create_task(
                _capture_awaitable(
                    cast("Callable[[], Coroutine[None, None, None]]", method)()
                )
            )
    ...
    async def _acquire_with_fork_tracking_async(self) -> None:
        with _fork_transition(self):
            ...
            try:
                ...
                BaseException
            ) as registration_error:  # cancellation must roll back the descriptor
                await self._rollback_failed_registration_async(registration_error)
                raise
        if self.is_locked:
            await self._invoke_on_acquired_async()

    async def _rollback_failed_acquire_async(
    ...
    async def _rollback_failed_registration_async(
    ...
    async def _invoke_on_acquired_async(self) -> None:
        ...
        try:
            ...
            BaseException
        ) as callback_error:  # caller control-flow errors must release the lock
        ...
        try:
            ...
            if (
                ...
                match errors:
                    ...
                    case (
                        asyncio.CancelledError() as cancellation,
                        backend_error,
                    ):  # pragma: no branch
                        _raise_grouped_errors(
                            _ASYNC_CONTEXT_RELEASE_ERRORS,
                            body_error,
                            cancellation,
                            backend_error,
                        )
...
class AsyncAcquireReturnProxy:
    ...
    ) -> None:  # ruff:ignore[undocumented-public-init]  # trivial release-on-exit proxy
        self.lock = lock
    ...
    async def __aenter__(
        ...
        BaseAsyncFileLock
    ):  # ruff:ignore[undocumented-magic-method]  # returns the wrapped lock
        return self.lock
    ...
    async def __aexit__(  # ruff:ignore[undocumented-magic-method]  # releases the wrapped lock on exit
        ...
        await self.lock._release_in_context(
            exc_value
        )  # ruff:ignore[private-member-access]  # releases the wrapped lock's context


class AsyncSoftFileLock(SoftFileLock, BaseAsyncFileLock):
    """Simply watches the existence of the lock file."""

    _lifetime_replacements: tuple[str, str] | None = (
