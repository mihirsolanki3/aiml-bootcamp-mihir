...
_CLOSE_ERROR_POLICIES: Final[frozenset[str]] = frozenset(
    {"default", "raise", "suppress"}
)
...
_REGISTER_AT_FORK: Final[_RegisterAtFork | None] = cast(
    "_RegisterAtFork | None", getattr(os, "register_at_fork", None)
)
...
def _detach_grouped_contexts(errors: tuple[BaseException, ...]) -> None:
    ...
    while pending:
        ...
        if (context := error.__context__) is not None and (
            ...
            or any(
                context is root or _contains_exception(root, context) for root in errors
            )
        ):
...
def _same_exception_tree(first: BaseException, second: BaseException) -> bool:
    ...
    while pending:
        ...
        if first_group.message != second_group.message or len(
            first_group.exceptions
        ) != len(second_group.exceptions):
            return False
        pending.extend(
            zip(first_group.exceptions, second_group.exceptions, strict=True)
        )
    return True
...
def _append_exception_context(error: BaseException, context: BaseException) -> None:
    if _exception_graph_contains(error, context) or _exception_graph_contains(
        context, error
    ):
        return
    ...
    while id(tail) not in seen:
        ...
        if (
            next_error := (
                tail.__cause__ if tail.__cause__ is not None else tail.__context__
            )
        ) is None:
...
def _exception_graph_contains(error: BaseException, target: BaseException) -> bool:
    ...
    while pending:
        ...
        if (
            id(current) in seen
        ):  # pragma: no cover - arbitrary caller exceptions can contain cycles
            continue
        seen.add(id(current))
...
def _grouped_errors(
    ...
    return (
        group.exceptions
        if group.message == message and getattr(group, marker[0], None) is marker[1]
        else None
    )
...
def _raise_chained_errors(
    ...
    if second_error is None:
        ...
        try:
            ...
            raise
    if (
        second_context := second_error.__context__
    ) is not None and second_context is not first_error:
    ...
    try:
        ...
        BaseException
    ):  # ruff:ignore[blind-except]  # first_error may be a control-flow exception
        first_error.__context__ = first_context
        try:
...
def _raise_body_and_release(
    ...
    _raise_grouped_errors(
        "lock body and release both failed", body_error, release_error
    )
...
_resolve_dir: Final[Callable[[str], str]] = (
    os.path.abspath if sys.platform == "win32" else os.path.realpath
)
...
def _canonical(path: str | os.PathLike[str]) -> str:
    ...
    return os.path.join(
        _resolve_dir(parent or os.curdir), name
    )  # ruff:ignore[os-path-join]  # string join matches abspath/realpath
...
class FileLockMeta(ABCMeta):
    ...
    def __call__(  # ruff:ignore[too-many-arguments]  # forwards the public constructor's documented parameters
        ...
        **kwargs: _ExtraValue,
        ...
        lifetime = _resolve_lifetime(
            lifetime, cls, stacklevel=cls._constructor_lifetime_warning_stacklevel
        )
        ...
        preserve_lock_file = _resolve_preserve_lock_file(
            preserve=preserve_lock_file,
            supported=cls._preserve_lock_file_supported,
            cls_name=cls.__name__,
        )
        on_acquired = _resolve_on_acquired(
            on_acquired, supported=cls._on_acquired_supported, cls_name=cls.__name__
        )
        ...
        with cls._instances_lock:
            if (instance := cls._instances.get(singleton_key)) is None:
                if (
                    singleton_key in cls._instances_under_construction
                ):  # pragma: needs fork
        ...
        params_to_check = {
            ...
            "mode": (
                mode,
                instance._context.mode,
            ),  # ruff:ignore[private-member-access]  # compares against the managed instance's own context
    ...
    def _create_instance(
        ...
        unsupported = sorted(
            ...
            and (
                (parameter := model.default_params.get(name)) is None
                or value != parameter.default
            )
        )
        if unsupported:
        ...
        return super().__call__(
            ...
            **{
                name: value
                for name, value in params.items()
                if name in model.accepted_params
            },
        )


_INIT_PARAMETER_MODELS: Final[
    WeakKeyDictionary[type[BaseFileLock], _InitParameterModel]
] = WeakKeyDictionary()
...
def _resolve_lifetime(
    lifetime: float | None, cls: type[BaseFileLock], *, stacklevel: int
) -> float | None:
    """
    ...
    if lifetime is not None:
        if isinstance(lifetime, bool) or not isinstance(lifetime, (int, float)):
            ...
            raise TypeError(msg)
        if lifetime < 0 or (
            isinstance(lifetime, float) and not math.isfinite(lifetime)
        ):
...
def _resolve_context_error_policy(policy: str) -> ContextErrorPolicy:
    ...
    if (
        ...
        try:
            ...
            ImportError
        ) as exc:  # pragma: no cover  # only on 3.10 without the exceptiongroup backport
...
def _resolve_preserve_lock_file(
...
class BaseFileLock(
    contextlib.ContextDecorator, metaclass=FileLockMeta
):  # ruff:ignore[too-many-public-methods]  # public config properties
    """
    ...
    _lifetime_unsupported_reason: str = (
        "a native OS lock cannot be broken safely by file age"
    )
    ...
    def __init__(  # ruff:ignore[too-many-arguments]  # public constructor: one parameter per documented lock option
        ...
        *,
        ...
        self._context_error_policy = (
            context_error_policy  # already validated by the metaclass
        )
        self._close_error_policy = (
            close_error_policy  # already validated by the metaclass
        )
        self._fallback_to_soft = fallback_to_soft
        self._preserve_lock_file = (
            preserve_lock_file  # already validated by the metaclass
        )
    ...
    def _close_released_fd(self, fd: int, *, default_suppresses: bool) -> None:
        ...
        try:
            ...
            if self._close_error_policy == "suppress" or (
                self._close_error_policy == "default" and default_suppresses
            ):
                return
            raise
    ...
    def release(
        ...
        None
    ):  # ruff:ignore[boolean-type-hint-positional-argument, boolean-default-value-positional-argument]  # public API: positional bool kept for backwards compatibility
        """
    ...
    def _descriptors_for_fork(
        self,
    ) -> tuple[tuple[int, tuple[int, int] | None], ...]:  # pragma: needs fork
        descriptors: list[tuple[int, tuple[int, int] | None]] = []
        if (
            self._context.lock_file_fd is not None
            and self._context.lock_file_fd_token is None
        ):
            descriptors.append(
                (self._context.lock_file_fd, self._context.lock_file_fd_identity)
            )
        if self._context.pending_lock_file_fd is not None:
            descriptors.append(
                (
                    self._context.pending_lock_file_fd,
                    self._context.pending_lock_file_fd_identity,
                )
            )
        return tuple(descriptors)

    def _raise_if_would_deadlock(
        self, canonical: str, *, timeout: float, blocking: bool
    ) -> None:
        """
        ...
        if would_block and _registry.held.get(self._registry_key(canonical)) not in {
            None,
            id(self),
        }:
            self._context.lock_counter -= 1
    ...
    def _registry_key(self, canonical: str) -> Hashable:
        return (
            canonical
            if (scope := self._deadlock_scope()) is None
            else (scope, canonical)
        )
    ...
    def _poll_delay(self, poll_interval: float, attempt: int) -> float:
        ...
        window = min(
            ...
            poll_interval * 2 ** min(attempt, _MAX_BACKOFF_EXPONENT),
        )  # pragma: needs hard-link
        return max(
            poll_interval, secrets.randbelow(int(window * 1_000_000) + 1) / 1_000_000
        )  # pragma: needs hard-link
    ...
    def _invoke_on_acquired(self) -> None:
        ...
        try:
            ...
            BaseException
        ) as callback_error:  # arbitrary caller code; roll back on any failure
            callback_context = callback_error.__context__
            try:
                self._release_with_fork_tracking()
            except (
                BaseException
            ) as release_error:  # ruff:ignore[blind-except]  # both errors surface via the group below
    ...
    def _rollback_failed_acquire(self, acquisition_error: BaseException) -> None:
        ...
        try:
            ...
            BaseException
        ) as error:  # ruff:ignore[blind-except]  # preserve registration and acquisition failures
            ...
            try:
                ...
                BaseException
            ) as error:  # ruff:ignore[blind-except]  # pragma: no cover - allocation/control-flow during fallback
                tracking_error = error
        try:
            self._release_with_fork_tracking()
        except (
            BaseException
        ) as rollback_error:  # ruff:ignore[blind-except]  # preserve rollback and acquisition failures
    ...
    def _rollback_failed_registration(
        ...
        try:
            ...
            BaseException
        ) as rollback_error:  # ruff:ignore[blind-except]  # preserve rollback and registration failures
            _raise_cleanup_errors(
                "descriptor registration cleanup failed",
                registration_error,
                tracking_error,
                rollback_error,
            )
        ...
        ):  # pragma: no cover - requires failed in-memory fallback
            _raise_cleanup_errors(
                "descriptor registration cleanup failed",
                registration_error,
                tracking_error,
    ...
    def _register_unverified_context_descriptor(self) -> None:
        ...
        if (
            ...
            self._context.lock_file_fd_token = _register_unverified_owned_descriptor(
                self._context.lock_file_fd
            )
    ...
    def _unregister_released_descriptor(self) -> None:
        if self._context.lock_file_fd is None:
            if (
                token := self._context.lock_file_fd_token
            ) is not None:  # pragma: needs fork
...
class AcquireReturnProxy:
    ...
    def __exit__(
        ...
        if isinstance(self.lock, BaseFileLock):
            self.lock._release_in_context(
                exc_value
            )  # ruff:ignore[private-member-access]  # forwards __exit__ to the owned lock's context release
...
_FORK_CLASSES: Final[WeakValueDictionary[int, _ForkResettableClass]] = (
    WeakValueDictionary()
)
...
def _register_owned_descriptor(
...
def _record_owned_descriptor(
    fd: int, identity: tuple[int, int] | None
) -> int:  # pragma: needs fork
...
def _pin_fork_objects() -> None:  # pragma: needs fork
    ...
    with _FORK_STATE.gate:
        _FORK_STATE.fork_owner_depths[thread_id] = (
            _FORK_STATE.fork_owner_depths.get(thread_id, 0) + 1
        )
        _FORK_STATE.admission_closed = True
        while _FORK_STATE.active_transitions > len(
            _FORK_STATE.transitions.get(thread_id, ())
        ):
    ...
    provisional_descriptor_tokens = tuple(
        starmap(
            ...
            (
                ...
                for descriptor in owner._descriptors_for_fork()
            ),  # ruff:ignore[private-member-access]  # snapshots each owner's own fork descriptors
        )
    )
    with _FORK_STATE.registry_lock:
        _FORK_STATE.provisional_descriptor_tokens.setdefault(thread_id, []).append(
            provisional_descriptor_tokens
        )
        _FORK_STATE.pinned_objects.setdefault(thread_id, []).append(
            tuple(_FORK_OBJECTS.values())
        )
        _FORK_STATE.pinned_classes.setdefault(thread_id, []).append(
            tuple(_FORK_CLASSES.values())
        )
...
def _resume_parent_after_fork() -> None:  # pragma: needs fork
    ...
    with _FORK_STATE.registry_lock:
        ...
        if not _FORK_STATE.provisional_descriptor_tokens[
            thread_id
        ]:  # pragma: needs fork
...
def _snapshot_descriptor_for_fork(
    fd: int, identity: tuple[int, int] | None
) -> int:  # pragma: needs fork
...
def _refresh_owner_pins() -> None:  # pragma: needs fork
    ...
    if (
        get_ident() not in fork_owner_thread_ids
    ):  # pragma: no cover - at-fork callbacks disable tracing
        return
    objects = tuple(_FORK_OBJECTS.values())
    classes = tuple(_FORK_CLASSES.values())
    for thread_id in fork_owner_thread_ids:
        if object_snapshots := _FORK_STATE.pinned_objects.get(
            thread_id
        ):  # pragma: needs fork
