...
def _probe_hard_link_unsupported_errnos() -> frozenset[int]:
    ...
    return frozenset(
        {ENOSYS, EXDEV} if not_supported is None else {ENOSYS, EXDEV, not_supported}
    )


_HARD_LINK_UNSUPPORTED_ERRNOS: Final[frozenset[int]] = (
    _probe_hard_link_unsupported_errnos()
)
...
class StrictSoftFileLock(BaseFileLock):
    ...
    _lifetime_unsupported_reason: str = (
        "a strict claim is never broken by age, only by force_break()"
    )
    ...
    def _acquire(self) -> None:
        ...
        if (
            sentinel_fd := _open_or_create_sentinel(
                self.lock_file, lock_path, self._open_mode()
            )
        ) is None:
            return
        try:
            ...
            BaseException
        ) as inspection_error:  # preserve inspection and descriptor cleanup errors
            try:
                os.close(sentinel_fd)
            except (
                BaseException
            ) as close_error:  # ruff:ignore[blind-except]  # preserve inspection and descriptor cleanup errors
                _raise_cleanup_errors(
                    "strict sentinel inspection cleanup failed",
                    inspection_error,
                    close_error,
                )
            raise
    ...
    def _attempt_doorway(
        ...
        try:
            publication_cleanup_error = _publish_record(
                intent_path, _claim_record(token), self._open_mode()
            )
        ...
        try:
            link_cleanup_error = _link_no_replace(
                claim_directory, intent_name, held_name
            )
        except (NotImplementedError, OSError) as error:
            _raise_if_hard_links_unsupported(self.lock_file, error)
            raise
        ...
        if (
            ...
            or min(_claim_token_key(claim.name) for claim in claims)
            != f"v1-{token}.claim"
        ):
            self._discard_doorway(sentinel_fd, sentinel_identity)
            return
    ...
    def _release(self) -> None:
        ...
        try:
            ...
            BaseException
        ) as close_error:  # ruff:ignore[blind-except]  # preserve claim and sentinel cleanup errors
    ...
    def _claim_directory(self) -> Path:
        ...
        return (
            Path(f"{_canonical(self.lock_file)}{_COORDINATION_SUFFIX}")
            / _CLAIM_DIRECTORY_NAME
        )
...
def _open_or_create_sentinel(lock_file: str, path: Path, mode: int) -> int | None:
    ...
    try:
        publication_cleanup_error = _publish_record(
            os.fspath(path), _LEGACY_SENTINEL, mode
        )
    except _PrivateRecordReclaimedError:
        return None
    except (NotImplementedError, OSError) as error:
...
def _read_claims(lock_file: str, directory: Path) -> tuple[StrictSoftFileClaim, ...]:
    try:
        ...
        reason = (
            f"cannot list claim directory: {error.strerror or type(error).__name__}"
        )
...
def _attempt_claim_read(
    ...
    try:
        ...
        if error.errno == ESTALE:
            return None, error
        reason = (
            f"cannot read claim: {error.strerror or str(error) or type(error).__name__}"
        )
        raise SoftFileLockProtocolError(lock_file, name, reason) from error


def _public_claim_names(
    ...
    for entry in entries:
        if not entry.name.startswith("."):
            ...
            public_name := _private_public_name(entry.name)
        ) is not None and _parse_claim_name(public_name) is not None:
...
def _parse_claim(
    ...
    try:
        magic, token, pid_text, hostname_hex, start_text, trailing = record.decode(
            "ascii"
        ).split("\n")
        pid = int(pid_text)
...
def _parse_claim_name(name: str) -> tuple[StrictSoftFileClaimState, str] | None:
    ...
    if (
        len(parts) != _CLAIM_NAME_PART_COUNT
        or parts[0] not in _CLAIM_STATES
        or parts[1] != "v1"
    ):
        return None
    token = parts[2]
    if len(token) != _TOKEN_HEX_LENGTH or any(
...
def _publish_record(
    ...
    try:
        _publish_record_in_directory(
            directory_ref, (private_name, public_name), mode, record
        )
    except (
        BaseException
    ) as publication_error:  # preserve publication and directory cleanup errors
        try:
            if directory_fd is not None:  # pragma: needs dir-fd
                os.close(directory_fd)
        except (
            BaseException
        ) as close_error:  # ruff:ignore[blind-except]  # pragma: needs dir-fd  # preserve publication and directory cleanup errors
            _raise_cleanup_errors(
                "strict publication directory cleanup failed",
                publication_error,
                close_error,
            )
        raise
    if directory_fd is not None:  # pragma: needs dir-fd
        try:
            ...
            BaseException
        ) as close_error:  # ruff:ignore[blind-except]  # caller records the published path before raising
            return close_error
    return None
...
def _publish_record_in_directory(
    ...
    ) is not None:  # pragma: needs o-nofollow
        flags |= o_nofollow
    ...
    try:
        ...
        BaseException
    ) as publication_error:  # preserve publication and cleanup errors
...
def _close_and_unlink_private_record(
    ...
    try:
        ...
        BaseException
    ) as error:  # ruff:ignore[blind-except]  # returned for grouping with unlink failures
        close_error = error
    ...
    try:
        ...
        BaseException
    ) as error:  # ruff:ignore[blind-except]  # returned for grouping with close failures
        unlink_error = error
...
def _raise_record_finalization_errors(
    ...
    if len(errors) > 1:
        _raise_cleanup_errors(
            "strict record finalization failed", errors[0], *errors[1:]
        )
    raise errors[0]
...
def _private_public_name(private_name: str) -> str | None:
    if not private_name.startswith(".") or not private_name.endswith(
        _PRIVATE_RECORD_SUFFIX
    ):
        return None
    public_name, marker, random_hex = private_name[
        1 : -len(_PRIVATE_RECORD_SUFFIX)
    ].rpartition(_PRIVATE_RECORD_MARKER)
    if (
...
def _reclaim_private_record(
    ...
    if not stat.S_ISREG(private_stat.st_mode):
        ...
        raise OSError(msg)
    if (
        private_stat.st_nlink == 1
        and now - private_stat.st_mtime < _PRIVATE_RECORD_GRACE
    ):
        return
    try:
...
def _unlink_private_record_once(
...
def _link_relative(
    ...
    if directory_fd is not None and _LINK_SUPPORTS_DIR_FD:  # pragma: needs dir-fd
        ...
        return
    _link_no_follow(
        Path(directory, source_name), Path(directory, destination_name)
    )  # pragma: win32 cover
...
def _link_no_replace(
    ...
    if _LINK_SUPPORTS_DIR_FD:  # pragma: needs dir-fd
        ...
        try:
            ...
            BaseException
        ) as link_error:  # preserve link and directory cleanup errors
            try:
                os.close(directory_fd)
            except (
                BaseException
            ) as close_error:  # ruff:ignore[blind-except]  # preserve link and directory cleanup errors
                _raise_cleanup_errors(
                    "strict link directory cleanup failed", link_error, close_error
                )
            raise
        try:
            os.close(directory_fd)
            ...
            BaseException
        ) as close_error:  # ruff:ignore[blind-except]  # caller records the held path before raising
            return close_error
        return None
    _link_no_follow(
        directory / source_name, directory / destination_name
    )  # pragma: win32 cover
...
def _unlink_owner_path_result(path: str) -> tuple[bool, BaseException | None]:
    try:
        ...
        BaseException
    ) as error:  # ruff:ignore[blind-except]  # keep ownership when unlink did not commit
        return False, error
...
def _unlink_in_directory(directory: Path, name: str) -> BaseException | None:
    if _UNLINK_SUPPORTS_DIR_FD:  # pragma: needs dir-fd
        ...
        try:
            ...
            BaseException
        ) as unlink_error:  # preserve unlink and directory cleanup errors
            try:
                os.close(directory_fd)
            except (
                BaseException
            ) as close_error:  # ruff:ignore[blind-except]  # preserve unlink and directory cleanup errors
                _raise_cleanup_errors(
                    "strict unlink directory cleanup failed", unlink_error, close_error
                )
            raise
        try:
            os.close(directory_fd)
            ...
            BaseException
        ) as close_error:  # ruff:ignore[blind-except]  # caller commits the removed path before raising
            return close_error
        return None
...
def _open_record(path: Path, limit: int) -> tuple[int, bytes]:
    ...
    try:
        ...
        try:
            ...
            BaseException
        ) as close_error:  # ruff:ignore[blind-except]  # preserve read and descriptor cleanup errors
            _raise_cleanup_errors(
                "strict record read cleanup failed", read_error, close_error
            )
        raise
    return fd, record


def _read_opened_record(
...
def _ensure_protocol_directory(lock_file: str, directory: Path) -> None:
    try:
        ...
        try:
            ...
            raise SoftFileLockProtocolError(
                lock_file, None, f"cannot inspect {directory}"
            ) from error
        if stat.S_ISDIR(mode) and not stat.S_ISLNK(mode):
            return
        raise SoftFileLockProtocolError(
            lock_file, None, f"{directory} is not a real directory"
        ) from None


def _validate_force_break_name(name: str) -> None:
    invalid_component = (
        not name or name.startswith(".") or name in {os.curdir, os.pardir}
    )
...
def _raise_if_hard_links_unsupported(
