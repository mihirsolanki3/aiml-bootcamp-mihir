...
) -> _RegistrySpec | None:  # ruff:ignore[any-type]
    with winreg.OpenKeyEx(
        company_key, tag
...
) -> tuple[str, str | None] | None:  # ruff:ignore[any-type]
    key_path = f"{hive_name}/{company}/{tag}"
    try:
        with winreg.OpenKeyEx(
            company_key, rf"{tag}\InstallPath"
        ) as ip_key, ip_key:  # ty: ignore[unresolved-attribute]
            if (exe := _resolve_exe(ip_key, key_path)) is not None and os.path.exists(
                exe
            ):
...
def parse_version(
    version_str: Any,
) -> tuple[int | None, int | None, int | None]:  # ruff:ignore[any-type]
