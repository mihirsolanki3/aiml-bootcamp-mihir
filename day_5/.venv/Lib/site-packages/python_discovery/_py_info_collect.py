...
UNSUPPORTED_EXIT_CODE = (
    79  # first value past the sysexits.h range, so no stdlib or shell meaning
)
UNSUPPORTED_MARKER = "unsupported Python version "

if (
    __name__ == "__main__" and sys.version_info[:2] < MIN_INTERROGATE_VERSION
):  # pragma: no cover # needs <3.6
    sys.stderr.write(
        UNSUPPORTED_MARKER + ".".join(str(i) for i in sys.version_info[:3])
    )
...
class PythonInfoCollector:
    ...
    def _init_sysconfig(self):
        ...
        confs = {
            k: (
                system_prefix if isinstance(v, str) and v.startswith(self.prefix) else v
            )
    ...
    def _sysconfig_path(self, key, config_var):
        ...
        if (
            pattern is None
        ):  # custom builds may ship schemes without stdlib / platstdlib
            return ""
    ...
    def _fast_get_system_executable(self):
        ...
        if os.path.exists(base_executable):  # pragma: >=3.11 cover
            ...
            if os.path.basename(base_executable) in ambiguous_names:
                ...
                if versioned is not None and not os.path.samefile(
    ...
    def _resolve_executable_symlink(self, path):
        ...
        if (
            self.os != "posix"
        ):  # pragma: win32 cover # CPython only does this where HAVE_READLINK
            return result
        if sysconfig.get_config_var(
            "PYTHONFRAMEWORK"
        ):  # macOS framework builds self-locate via dyld; e.g. Homebrew
    ...
    def _distutils_install():  # pragma: <3.11 cover # 3.11+ uses the "venv" scheme instead
        ...
        return {
            key: (getattr(install, "install_{}".format(key))[1:]).lstrip(os.sep)
