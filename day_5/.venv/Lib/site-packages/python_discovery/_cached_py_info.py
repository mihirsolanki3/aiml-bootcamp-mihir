    ...
    with py_info_store.locked():
        ...
        if py_info is None:
            ...
            if failure is not None and not isinstance(
                failure, _UnsupportedInterpreterError
            ):
                _LOGGER.debug(
            ...
            if isinstance(failure, _UnsupportedInterpreterError):
                _LOGGER.warning(
                    "%s", failure
                )  # the verdict is permanent, warn once and remember it
...
def _query_failure(
    exe: str, out: str, err: str | None, code: int | None
) -> RuntimeError:
    ...
    if (
        code == UNSUPPORTED_EXIT_CODE
        and err is not None
        and (at := err.find(UNSUPPORTED_MARKER)) != -1
    ):
