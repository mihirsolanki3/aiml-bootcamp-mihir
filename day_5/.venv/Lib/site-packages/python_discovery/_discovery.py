...
def propose_interpreters(
    ...
    *,
    ...
    yield from _propose_from_uv(
        spec, cache, env, all_implementations=all_implementations
    )
...
def _propose_current_and_windows(
    ...
    if IS_WIN:  # pragma: win32 cover
        from ._windows import (
            propose_interpreters as win_propose,
...
def _propose_from_uv(
    ...
    *,
    ...
    for root in _uv_python_roots(env):
        for install in _uv_installs(
            root, spec, all_implementations=all_implementations
        ):
...
def _uv_state_dirs(env: Mapping[str, str]) -> Generator[Path, None, None]:
    ...
    if windows:
        app_data = (
            Path(env["APPDATA"]) if "APPDATA" in env else home / "AppData" / "Roaming"
        )
    ...
    if (xdg_data_home := env.get("XDG_DATA_HOME")) and (
        xdg := Path(xdg_data_home)
    ).is_absolute():
        yield xdg / "uv"
    else:
...
def _uv_installs(
    root: Path, spec: PythonSpec, *, all_implementations: bool
) -> list[_UvInstall]:
    try:
        matching = [
            install
            ...
            and entry.is_dir()
            and _uv_install_matches(
                install := _parse_uv_install(entry),
                spec,
    ...
    matching.sort(
        key=_uv_sort_key
    )  # stable, so equally ranked installs stay in directory name order
    return matching
...
def _parse_uv_install(path: Path) -> _UvInstall:
    ...
    try:
        ...
        ValueError
    ):  # not a uv key, but the directory may still hold an interpreter worth probing
...
def _uv_install_matches(
    install: _UvInstall, spec: PythonSpec, *, all_implementations: bool
) -> bool:
...
def _uv_version_matches(key: _UvKey, spec: PythonSpec) -> bool:
    ...
    if key.implementation not in {"cpython", "pypy"}:
        return True
    if spec.version_specifier is not None and not spec.version_specifier.contains(
        str(key.version)
    ):
        return False
    return all(
        req is None or our == req
        for our, req in zip(key.version.release, (spec.major, spec.minor, spec.micro))
    )
...
def _uv_sort_key(install: _UvInstall) -> tuple[int, ...]:
    ...
    return (
        (
            impl_order.index(key.implementation)
            if key.implementation in impl_order
            else len(impl_order)
        ),
...
def path_exe_finder(
    ...
    pat = spec.generate_re(
        windows=sys.platform == "win32", all_implementations=all_implementations
    )
