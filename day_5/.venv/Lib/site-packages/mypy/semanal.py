...
class SemanticAnalyzer(
    ...
    def __init__(
        ...
        self.type_expression_parse_count: int = (
            0  # Total try_parse_as_type_expression calls
        )
    ...
    def refresh_top_level(self, file_node: MypyFile) -> None:
        ...
        if self.options.allow_redefinition and not self.options.local_partial_types:
            ...
            n.end_column = 0
            self.fail(
                "--local-partial-types must be enabled if using --allow-redefinition", n
            )
            ...
            if name == "__doc__":
                ...
                if inst is None:
                    if (
                        ...
                        or self.options.clone_for_module(
                            "importlib.machinery"
                        ).follow_imports
                        == "skip"
                    ):
    ...
    def visit_decorator(self, dec: Decorator) -> None:
        ...
        if not no_type_check and (
            self.recurse_into_functions or dec.func.def_or_infer_vars
        ):
    ...
    def analyze_typeddict_classdef(self, defn: ClassDef) -> bool:
        if (
            ...
            is_typeddict, info = self.typed_dict_analyzer.analyze_typeddict_classdef(
                defn
            )
        if is_typeddict:
            if info is None:
    ...
    def analyze_class_decorator_common(
        ...
        if refers_to_fullname(decorator, FINAL_DECORATOR_NAMES):
            ...
            if info.is_protocol:
                self.fail(
                    "@disjoint_base cannot be used with protocol class", decorator
                )
    ...
    def clean_up_bases_and_infer_type_variables(
        ...
        list[Expression], list[TypeVarLikeType], bool, list[tuple[str, TypeVarLikeExpr]]
    ]:
            ...
            for p in defn.type_args:
                ...
                if isinstance(node.node, TypeVarTupleExpr):
                    if has_type_var_tuple:
                        self.fail(
                            "Can only use one type var tuple in a class def", context
                        )
                        continue
    ...
    def tvar_defs_from_tvars(
        ...
        for name, tvar_expr in tvars:
            ...
            if not tvar_expr.is_new_style and not self.tvar_scope.allow_binding(
                ...
                self.fail(
                    message_registry.TYPE_VAR_REDECLARED_IN_NESTED_CLASS.format(name),
                    context,
                )
    ...
    def analyze_alias(
        ...
        set[str],
        bool,
        dict[str, set[TypeAlias | TypeInfo]],
    ]:
        ...
        indexed = bool(
            isinstance(typ, UnboundType) and (typ.args or typ.empty_tuple_index)
        )
        default_depends = {}
        ...
        if not s.type.args:
            if isinstance(s.rvalue, TempNode) and s.rvalue.no_rhs:
                if self.options.disallow_any_generics:
                    self.fail(
                        "ClassVar without type argument becomes Any",
                        s,
                        code=codes.TYPE_ARG,
                    )
                return
            ...
            if not is_same_symbol(old, new):
                ...
                if isinstance(old, Var) and is_init_only(old):
                    if old.has_explicit_value:
                        self.fail(
                            "InitVar with default value cannot be redefined", context
                        )
                elif (
                    not (
    ...
    def _get_names_in_scope(self) -> set[str]:
        ...
        if b:
            ...
            for builtin_name in b.node.names.keys():
                if not (
                    len(builtin_name) > 1
                    and builtin_name[0] == "_"
                    and builtin_name[1] != "_"
                ):
    ...
    def try_parse_as_type_expression(self, maybe_type_expr: Expression) -> None:
        ...
        if isinstance(maybe_type_expr, (NameExpr, MemberExpr)):
            ...
            if str_value.isidentifier():
                sym = self.lookup(
                    str_value, UnboundType(str_value), suppress_errors=True
                )
                if sym is None:
            ...
            if isinstance(maybe_type_expr.base, NameExpr):
                ...
                if isinstance(leftmost, NameExpr):
                    if isinstance(
                        leftmost.node, Var
                    ) and not self.var_is_typing_special_form(leftmost.node):
                        # Leftmost part of IndexExpr refers to a Var. Not a valid type.
                        maybe_type_expr.as_type = None
                        return
                else:
