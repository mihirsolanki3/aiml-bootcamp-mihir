...
def expand_and_bind_callable(
    ...
    if var.is_settable_property and mx.is_lvalue and var.setter_type is not None:
        if expanded.variables:
            type_ctx = mx.rvalue or TempNode(
                AnyType(TypeOfAny.special_form), context=mx.context
            )
...
def check_self_arg(
    ...
    for item in items:
        ...
        if isinstance(selfarg, Instance) and isinstance(
            p_dispatched_arg_type, Instance
        ):
    ...
    *,
    ...
    if node.implicit and isinstance(node.node, Var):
        if node.node.is_final:
            ...
            mx.fail(
                message_registry.CANNOT_ACCESS_INSTANCE_ONLY_ATTR.format(node.node.name)
            )
    ...
    if isinstance(node.node, (TypeInfo, TypeAlias, MypyFile, TypeVarLikeExpr)):
        ...
        return mx.chk.expr_checker.analyze_static_reference(
            node.node, mx.context, mx.is_lvalue
        )
        ...
        return Overloaded(
            [
                cast(
                    CallableType,
                    add_class_tvars(
                        item, isuper, is_classmethod, mx, original_vars=original_vars
                    ),
                )
                for item in t.items
            ]
...
def instance_fallback(typ: ProperType) -> Instance:
    ...
    if instance_cache.object_type is None:
        object_typeinfo = lookup_stdlib_typeinfo(
            "builtins.object", modules_state.modules
        )
    ...
    for base in info.mro[1:]:
        if (node := base.names.get(name)) is not None:
            if (
                ...
                and node.node.has_explicit_value
            ):
                return True
    return False
