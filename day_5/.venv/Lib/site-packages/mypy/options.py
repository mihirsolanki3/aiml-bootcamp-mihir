...
OPTIONS_AFFECTING_CACHE_NO_PLATFORM: Final = tuple(
...
COMPLETE_FEATURES: Final = frozenset(
    (TYPE_VAR_TUPLE, UNPACK, NEW_GENERIC_SYNTAX, TYPE_FORM)
)
...
class Options:
    ...
    def process_error_codes(self, *, error_callback: Callable[[str], Any]) -> None:
        ...
        if invalid_code_names_here:
            error_callback(
                f"Invalid error code(s): {', '.join(sorted(invalid_code_names_here))}"
            )
