...
class TypedDictAnalyzer:
    ...
    def analyze_typeddict_classdef(
        ...
        if (
            ...
            field_types = {
                key: source.field_type for (key, source) in field_sources.items()
            }
            required_keys = {
                key for (key, source) in field_sources.items() if source.is_required
            }
            readonly_keys = {
                key for (key, source) in field_sources.items() if source.is_readonly
            }
    ...
    def verify_requiredness_compatibility(
        ...
        if source.is_required and not is_required:
            if primary_source_base is None:
                self.fail(
                    f'Field "{field_name}" is required in base class "{source.base.name}"',
                    ctx,
                )
            else:
                self.fail(
            ...
            if primary_source_base is None:
                self.fail(
                    f'Field "{field_name}" can be deleted in base class "{source.base.name}"',
                    ctx,
                )
            else:
                self.fail(
    ...
    def resolve_field_inheritance(
        ...
        dict[str, Type], set[str], set[str], bool, dict[str, TypedDictFieldSource]
    ]:
        """Determine field types, requiredness, readonlyness, and closedness."""
        field_sources = self.field_sources_in_reverse_order(
            bases, child_field_sources, ctx
        )
    ...
    def check_typeddict(
        ...
        if not ok:
            ...
            info = self.build_typeddict_typeinfo(
                name, {}, set(), set(), False, call.line, None
            )
        else:
            ...
            for field, t in zip(items, wrapped_types):
                unwrapped_type, is_required, is_readonly = self.extract_meta_info(
                    t, node
                )
