...
def build(
    ...
    if options.num_workers > 0:
        ...
        options_data = os_path_join(
            options.cache_dir, f".worker_options.{build_id}.data"
        )
        with open(options_data, "wb") as f:
            f.write(options.to_bytes())
        workers = [
            WorkerClient(
                f".mypy_worker.{build_id}.{idx}.json",
                options_data,
                worker_env or os.environ,
            )
    ...
    try:
        ...
        for worker in workers:
            ...
            try:
                send(
                    worker.conn,
                    SccRequestMessage(scc_ids=[], import_errors={}, mod_data={}),
                )
    ...
    if options.warn_unused_configs and unused_configs and not options.non_interactive:
        unused = get_config_module_names(
            ...
            [
                ...
                if glob in unused_configs
            ],
        )
        flush_errors(
            None,
            ["{}: note: unused section(s): {}".format(options.config_file, unused)],
            False,
        )
...
class BuildManager:
    ...
    def __init__(
        ...
        modules_state.node_fixer = NodeFixer(
            self.modules, self.options.use_fine_grained_cache
        )
        ...
        for module in CORE_BUILTIN_MODULES:
            ...
            self.error(
                None, f'This file shadows library module "{module}"', blocker=True
            )
            self.note(
                None,
                f'A user-defined top-level module with name "{module}" is not supported',
            )
    ...
    def parse_all(self, states: list[State], post_parse: bool = True) -> None:
        ...
        for state in states:
            if not self.fscache.exists(state.xpath, real_only=True) or (
                self.shadow_map
                and self.maybe_swap_for_shadow_path(state.xpath) != state.xpath
            ):
        ...
        if len(parallel_states) > 1:
            ...
            parallel_parsed_states, parallel_parsed_states_set = (
                self.parse_files_threaded_raw(parallel_states)
            )
            ...
            for state in parallel_states:
                ...
                if state in parallel_parsed_states_set:
                    if state.tree.raw_data is not None:
                        ...
                        state.get_source()
                    state.early_errors = list(
                        self.errors.error_info_map.get(state.xpath, [])
                    )
                    state.semantic_analysis_pass1()
                    self.ast_cache[state.id] = (
                        state.tree,
                        state.early_errors,
                        state.source_hash,
                    )
        ...
        with ThreadPoolExecutor(max_workers=min(available_threads, 8)) as executor:
            for state in states:
                ...
                if state.id not in self.ast_cache:
                    ...
                    futures.append(
                        executor.submit(state.parse_file_inner, state.source)
                    )
                    ...
                    state.tree, state.early_errors, source_hash = self.ast_cache[
                        state.id
                    ]
    ...
    def receive_worker_message(self, idx: int) -> ReadBuffer:
        ...
        try:
            ...
            try:
                ...
                exit_code = None
            exit_status = (
                f"exit code {exit_code}" if exit_code is not None else "still running"
            )
            raise OSError(
    ...
    def error(
        ...
        *,
        ...
        if line is None:
            ...
            column = 0
        self.errors.report(
            line, column, msg, code, blocker=blocker, only_once=only_once
        )
    ...
    def note(
        ...
        *,
        ...
        if line is None:
            ...
            column = 0
        self.errors.report(
            line, column, msg, code, severity="note", only_once=only_once
        )
...
def write_deps_cache(
    ...
    if error:
        ...
        manager.error(
            None, "Error writing fine-grained dependencies cache", blocker=True
        )
...
def find_cache_meta(
    ...
    if manager.options.fixed_format_cache:
        meta_ex = _load_ff_file(
            meta_ex_file,
            manager,
            log_error_fmt="Could not load meta_ex for {}: ",
            id=id,
        )
    else:
...
class State:
        ...
        if meta:
            ...
            if not manager.use_fine_grained_cache():
                ...
                if exist_added_packages(suppressed, manager):
                    state.needs_parse = (
                        True  # This is safe because the cache is anyway stale.
                    )
                ...
                if exist_removed_submodules(dependencies, manager):
                    state.needs_parse = (
                        True  # Same as above, the current state is stale anyway.
                    )
    ...
    def read(cls, buf: ReadBuffer, manager: BuildManager) -> State:
        ...
        import_context = [
            (read_str(buf), read_int(buf)) for _ in range(read_int_bare(buf))
        ]
        ...
        priorities = {
            read_str_bare(buf): read_int(buf) for _ in range(read_int_bare(buf))
        }
        assert read_tag(buf) == DICT_STR_GEN
        dep_line_map = {
            read_str_bare(buf): read_int(buf) for _ in range(read_int_bare(buf))
        ...
        assert read_tag(buf) == DICT_STR_GEN
        dep_hashes = {
            read_str_bare(buf): read_bytes(buf) for _ in range(read_int_bare(buf))
        }
        return cls(
            manager=manager,
            order=order,
    ...
    def reload_meta(self) -> None:
        ...
        new_meta_pair = find_cache_meta(
            self.id, self.path, self.manager, skip_validation=True
        )
            ...
            if self.path and source is None:
                try:
                    ...
                    if self.path.endswith(".pyd"):
                        ...
                        err = (
                            f"{self.path}: error: Cannot decode file: {str(decodeerr)}"
                        )
                    raise CompileError(
    ...
    def parse_file_inner(
        self, source: str | None, raw_data: FileRawData | None = None
    ) -> None:
        t0 = time_ref()
    ...
    def check_for_invalid_options(self) -> None:
        if self.options.mypyc and not self.options.strict_bytes:
            ...
            self.manager.error(
                None,
                "Option --strict-bytes cannot be disabled when using mypyc",
                blocker=True,
            )
    ...
    def compute_dependencies(self) -> None:
        ...
        self.imports_ignored = {
            line: codes
            for line, codes in self.tree.ignored_lines.items()
    ...
    def type_check_first_pass(self, recurse_into_functions: bool = True) -> None:
        ...
        with self.wrap_context():
            self.type_checker().check_first_pass(
                recurse_into_functions=recurse_into_functions
            )
        self.time_spent_us += time_spent_us(t0)
    ...
    def patch_indirect_dependencies(
    ...
    def generate_unused_ignore_notes(self) -> None:
        if (
            ...
            is_typeshed = self.tree is not None and self.tree.is_typeshed_file(
                self.options
            )
            with self.wrap_context():
                self.manager.errors.generate_unused_ignore_errors(
                    self.xpath, is_typeshed
                )
...
def in_partial_package(id: str, manager: BuildManager) -> bool:
    ...
    while "." in id:
        ...
        if ancestor_mod is not None:
            ...
            manager.known_partial_packages[ancestor] = (
                ancestor_mod.is_partial_stub_package
            )
...
def module_not_found(
    ...
    errors.set_file_ignored_lines(
        caller_state.xpath,
        (
            caller_state.tree.ignored_lines
            if caller_state.tree
            else caller_state.imports_ignored
        ),
...
def dispatch(
    ...
    if not manager.use_fine_grained_cache():
        ...
        for thread in connect_threads:
            thread.join()
        not_connected = [
            str(idx) for idx, wc in enumerate(manager.workers) if not wc.connected
        ]
        if not_connected:
            raise OSError(
                f"Cannot connect to build worker(s): {', '.join(not_connected)}"
            )
    ...
    for st in new:
        ...
        for dep in st.ancestors + dependencies + st.suppressed:
            ...
            if ignored and dep not in added:
                ...
                if (
                    not manager.use_fine_grained_cache()
                    or manager.options.warn_unused_configs
                ):
                ...
                try:
                    ...
                    if newst.path:
                        ...
                        if newst_path in seen_files:
                            manager.errors.set_file(
                                newst.xpath, newst.id, manager.options
                            )
                            manager.error(
                                None,
    ...
    for ascc in sccs:
        ...
        for id in ascc.mod_ids:
            for dep in graph[id].dep_hashes:
                if (
                    dep in graph
                    and graph[dep].interface_hash != graph[id].dep_hashes[dep]
                ):
                    stale_deps.add(dep)
        ...
        if manager.logging_enabled:
            if fresh:
                ...
                fresh_msg = (
                    f"stale due to stale indirect dep(s): first {stale_indirect}"
                )
        ...
        if fresh:
            ...
            if manager.logging_enabled:
                ...
                if size == 1:
                    ...
                    manager.log(
                        "Scheduling SCC of size %d (%s) as %s"
                        % (size, scc_str, fresh_msg)
                    )
...
def process_graph(graph: Graph, manager: BuildManager) -> None:
    ...
    manager.log(
        ...
        % (len(sccs), max(len(scc.mod_ids) for scc in sccs))
    )
    ...
    manager.trace(
        f"Transitive deps cache size: {sys.getsizeof(manager.transitive_deps_cache)}"
    )
...
def process_stale_scc(graph: Graph, ascc: SCC, manager: BuildManager) -> None:
    ...
    for id in stale:
        if graph[id].xpath not in manager.errors.ignored_files:
            ...
            manager.flush_errors(
                manager.errors.simplify_path(graph[id].xpath), formatted, False
            )
...
def process_stale_scc_interface(
    ...
    for id in stale:
        ...
        manager.commit_module(meta_file)
        scc_result.append(
            (id, ModuleResult(graph[id].interface_hash.hex(), []), meta_file)
        )
    manager.done_sccs.add(ascc.id)
    manager.add_stats(
        load_missing_time=t1 - t0,
...
def prepare_sccs_full(
    ...
    for scc in sccs:
        for id in scc.mod_ids:
            scc_deps_map.setdefault(scc, set()).update(
                scc_map[dep] for dep in edges[id]
            )
    for scc in sccs:
...
def sorted_components(graph: Graph) -> list[SCC]:
    ...
    scc_dep_map = prepare_sccs_full(
        strongly_connected_components(vertices, edges), edges
    )
    # Topsort.
    res = []
    for ready in topsort(scc_dep_map):
        ...
        sorted_ready = sorted(
            ready, key=lambda scc: -min(graph[id].order for id in scc.mod_ids)
        )
    ...
    def read(cls, buf: ReadBuffer) -> SccResponseMessage:
        ...
        if tag == LITERAL_NONE:
            return SccResponseMessage(
                ...
                blocker=CompileError(
                    read_str_list(buf), read_bool(buf), read_str_opt(buf)
                ),
            )
        else:
            ...
            return SccResponseMessage(
                ...
                is_interface=is_interface,
                result={
                    read_str_bare(buf): ModuleResult.read(buf)
                    for _ in range(read_int_bare(buf))
                },
            )
...
class GraphMessage(IPCMessage):
    ...
    def read(cls, buf: ReadBuffer, manager: BuildManager | None = None) -> GraphMessage:
        ...
        graph = {
            read_str_bare(buf): State.read(buf, manager)
            for _ in range(read_int_bare(buf))
        }
        missing_modules = {
            read_str_bare(buf): read_int(buf) for _ in range(read_int_bare(buf))
