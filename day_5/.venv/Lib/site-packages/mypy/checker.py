...
class PartialTypeScope:
    def __init__(
        self, map: dict[Var, Context], is_function: bool, is_local: bool
    ) -> None:
        self.map: Final = map
...
class TypeChecker(NodeVisitor[None], TypeCheckerSharedApi, SplittingVisitor):
    ...
    def check_partial_impl(self, impl: FuncDef | Decorator) -> None:
        ...
        if isinstance(impl, FuncDef):
            ...
            if any(
                refers_to_fullname(d, "typing.no_type_check") for d in impl.decorators
            ):
                return
    ...
    def check_default_params(
        self, item: FuncItem, body_is_trivial: bool | None = None
    ) -> None:
    ...
    def visit_class_def(self, defn: ClassDef) -> None:
        ...
        with (
            ...
            if not self.can_skip_diagnostics:
                if not (
                    defn.info.typeddict_type
                    or defn.info.tuple_type
                    or defn.info.is_enum
                ):
    ...
    def check_protocol_variance(self, defn: ClassDef) -> None:
        ...
        for i, tvar in enumerate(tvars):
            if not isinstance(tvar, TypeVarType):
                ...
                continue
            up_args: list[Type] = [
                object_type if i == j else dummy.copy_modified()
                for j, _ in enumerate(tvars)
            ]
            down_args: list[Type] = [
                UninhabitedType() if i == j else dummy.copy_modified()
                for j, _ in enumerate(tvars)
            ]
                ...
                if isinstance(lvalue_type, PartialType) and lvalue_type.type is None:
                    ...
                    if is_valid_inferred_type(
                        ...
                        if not self.infer_partial_type(var, lvalue, rvalue_type):
                            ...
                            self.set_inferred_type(
                                var, lvalue, make_optional_type(fallback)
                            )
                elif (
    ...
    def infer_rvalue_with_fallback_context(
        ...
        with (
            ...
            rvalue_type = self.expr_checker.accept(
                rvalue,
                type_context=preferred_context,
        ...
        argument_redefinition_fallback = (
            inferred is not None
            and inferred.is_argument
            and local_errors.has_new_errors()
        )
        ...
        try_fallback = (
            redefinition_fallback or union_fallback or argument_redefinition_fallback
        )
    ...
    def check_simple_assignment(
        ...
        *,
        ...
        if self.is_stub and isinstance(rvalue, EllipsisExpr):
            ...
            try_fallback = (
                inferred is not None
                or isinstance(get_proper_type(lvalue_type), UnionType)
            ) and not self.simple_rvalue(rvalue)

            if (
                not try_fallback
                or lvalue_type is None
                or is_typeddict_type_context(lvalue_type)
            ):
                rvalue_type = self.expr_checker.accept(
    ...
    def infer_context_dependent(
        ...
        with self.msg.filter_errors(
            filter_deprecated=True, save_filtered_errors=True
        ) as msg:
            with (
                self.local_type_map as type_map,
        ...
        if defn is not None:
            ...
            if s.expr:
                ...
                if (
                    isinstance(
                        s.expr,
                        (CallExpr, ListExpr, TupleExpr, DictExpr, SetExpr, OpExpr),
                    )
                ...
                if (
                    isinstance(typ, Instance)
                    and typ.type.fullname in NOT_IMPLEMENTED_TYPE_NAMES
                ):
        ...
        with self.binder.frame_context(can_skip=False, fall_through=0):
            ...
            for p, g, b in zip(s.patterns, s.guards, s.bodies):
                ...
                with self.binder.frame_context(can_skip=True, fall_through=2):
                    ...
                    self.check_and_remove_capture_conflicts(
            ...
            if (
                not is_unreachable_map(unmatched_types)
                and not self.current_node_deferred
            ):
    ...
    def intersect_instances(
        ...
        curr_module.names[full_name] = SymbolTableNode(
            GDEF, info, False, module_hidden=True
        )
        return Instance(
    ...
    def comparison_type_narrowing_helper(
        ...
        for i, expr in enumerate(operands):
            ...
            if (
                ...
                and not (
                    isinstance(p_expr, TypeType)
                    and isinstance(p_expr.item, TypeVarType)
                )
            ):
                h = literal_hash(expr)
                if h is not None:
                ...
                if left_index in narrowable_operand_index_to_hash:
                    ...
                    if (
                        ...
                        if isinstance(container_expr, (ListExpr, SetExpr)):
                            if all(
                                not isinstance(i, StarExpr)
                                for i in container_expr.items
                            ):
                    ...
                    if container_item_types is not None:
                        ...
                        else_map = reduce_and_conditional_type_maps(
                            all_else_maps, use_meet=True
                        )
                    else:
                        collection_item_type = get_proper_type(
                            builtin_item_type(p_iterable_type)
                        )
    ...
    def narrow_type_by_identity_equality(
        ...
        if is_identity_comparison:
            ...
            custom_eq_indices = {
                i for i in expr_indices if has_custom_eq_checks(operand_types[i])
            }
        else:
        ...
        for i in expr_indices:
            ...
            for j in expr_indices:
                ...
                narrowable_expr_type, ambiguous_expr_type = (
        ...
        for i in custom_eq_indices:
            ...
            for expr_type in union_expr_type.items:
                ...
                expr_type = coerce_to_literal(
                    try_expanding_sum_type_to_union(expr_type, None)
                )
                for j in expr_indices:
                    if j in custom_eq_indices:
                        if i == j:
                    ...
                    if narrowable_expr_type is None:
                        ...
                        if ambiguous_expr_type is not None:
                            if_type = make_simplified_union(
                                [if_type, ambiguous_expr_type]
                            )
                            else_type = make_simplified_union(
                                [else_type, ambiguous_expr_type]
                            )
        ...
        for i in expr_indices:
            ...
            if isinstance(p_expr_type, TypeType) and isinstance(
                p_expr_type.item, TypeVarType
            ):
        ...
        if_map = reduce_and_conditional_type_maps(
            all_if_maps, use_meet=len(operands) > 2
        )
                        ...
                        def replay_lookup(new_parent_type: ProperType) -> Type | None:
                            if isinstance(new_parent_type, TupleType):
                                try:
                                    ...
                                    member_types = [
                                        new_parent_type.items[key]
                                        for key in int_literals
                                    ]
                                except IndexError:
                                    return None
                                return make_simplified_union(member_types)
...
def conditional_types(
    ...
    *,
    ...
    proposed_type = make_simplified_union(
        [type_range.item for type_range in proposed_type_ranges]
    )
    items = (
        proposed_type.items if isinstance(proposed_type, UnionType) else [proposed_type]
    )
        ...
        if not use_meet:
            ...
            if isinstance(pt1, UninhabitedType):
                ...
                if isinstance(pt2, AnyType) and not (
                    ...
                    and any(
                        isinstance(get_proper_type(item), AnyType) for item in pt1.items
                    )
                ):
                    result[e1] = t1
    ...
    if isinstance(t, UnionType):
        return [
            UnionType.make_union(
                [b for a in t.items for b in flatten_types_if_tuple(a)]
            )
        ]
...
def is_overlapping_types_for_overload(left: Type, right: Type) -> bool:
    ...
    return is_overlapping_types(
        left, right, ignore_promotions=True, overlap_for_overloads=True
    )
...
OPEN_VALUE_EQUALITY_DOMAIN_NAMES: Final = frozenset(
    OPEN_VALUE_EQUALITY_DOMAINS.values()
)
...
def equality_value_info(t: Type) -> EqualityValueInfo:
    t = get_proper_type(t)
    if isinstance(t, UnionType):
        return combine_equality_value_info(
            equality_value_info(item) for item in t.items
        )
    if isinstance(t, TypeVarType):
        if t.values:
            return combine_equality_value_info(
                equality_value_info(item) for item in t.values
            )
...
def combine_equality_value_info(
