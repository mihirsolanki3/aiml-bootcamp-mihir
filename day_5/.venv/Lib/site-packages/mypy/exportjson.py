...
def convert_var(self: Var) -> Json:
    data: dict[str, Any] = {
        ...
        "type": None if self.type is None else convert_type(self.type),
        "setter_type": (
            None if self.setter_type is None else convert_type(self.setter_type)
        ),
...
def convert_type_info(self: TypeInfo, cfg: Config) -> Json:
    data = {
        ...
        "alt_promote": (
            None if self.alt_promote is None else convert_type(self.alt_promote)
        ),
        "declared_metaclass": (
            None
            if self.declared_metaclass is None
            else convert_type(self.declared_metaclass)
        ),
        "metaclass_type": (
            None if self.metaclass_type is None else convert_type(self.metaclass_type)
        ),
        "tuple_type": (
            None if self.tuple_type is None else convert_type(self.tuple_type)
        ),
        "typeddict_type": (
            None
            if self.typeddict_type is None
            else convert_typeddict_type(self.typeddict_type)
        ),
        ...
        "self_type": (
            convert_type(self.self_type) if self.self_type is not None else None
        ),
        "dataclass_transform_spec": (
...
def convert_instance(self: Instance) -> Json:
    ...
    data["extra_attrs"] = (
        convert_extra_attrs(self.extra_attrs) if self.extra_attrs else None
    )
    return data
...
def convert_any_type(self: AnyType) -> Json:
    return {
        ...
        "source_any": (
            convert_type(self.source_any) if self.source_any is not None else None
        ),
...
def convert_callable_type(self: CallableType) -> Json:
    return {
        ...
        "type_guard": (
            convert_type(self.type_guard) if self.type_guard is not None else None
        ),
...
def convert_binary_cache_meta_to_json(data: bytes, data_file: str) -> Json:
    ...
    assert (
        meta is not None
    ), f"Error reading meta cache file associated with {data_file}"
    return {
        "id": meta.id,
        "path": meta.path,
        "mtime": meta.mtime,
