...
def verify_mypyfile(
    ...
    for entry in sorted(to_check):
        ...
        if entry in stub.names:
            ...
            elif isinstance(stub_entry, nodes.SymbolNode) and (
                name := stub_entry.fullname
            ):
            ...
            if (
                orig_module
                and orig_module != stub.fullname
                and orig_module in _all_stubs
            ):
...
class Signature(Generic[T]):
        ...
        for stub_arg in stub_args:
            if stub_arg.kind.is_positional():
                ...
                if stub_arg.variable.type is not None and isinstance(
                    (
                        typed_dict_arg := mypy.types.get_proper_type(
                            stub_arg.variable.type
                        )
                    ),
    ...
    def from_overloadedfuncdef(
        ...
        for func in map(_resolve_funcitem_from_decorator, stub.items):
            assert (
                func is not None
            ), f"Failed to resolve decorated overload of {stub.fullname!r}"
...
def _resolve_funcitem_from_callable_type(
    ...
    ret = nodes.FuncDef(
        name=typ.name or "", arguments=args, body=nodes.Block([]), typ=typ
    )
...
def is_read_only_property(runtime: object) -> bool:
    return (
        isinstance(runtime, property) and runtime.fset is None and runtime.fdel is None
    )
    ...
    for module in modules:
        for error in test_module(module):
            ...
            if args.ignore_disjoint_bases and error.is_disjoint_base_related():
                continue
            if (
                not args.strict_type_check_only
