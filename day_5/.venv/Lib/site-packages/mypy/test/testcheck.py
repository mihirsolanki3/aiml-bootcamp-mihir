...
try:
    if sys.version_info >= (3, 14) and bool(
...
class TypeCheckSuite(DataSuite):
    ...
    def verify_cache(
        ...
        if not blocker:
            ...
            modules.update(
                {module_name: path for module_name, path, text in module_data}
            )
            missing_paths = self.find_missing_cache_files(modules, manager)
            if missing_paths:
                raise AssertionError(
                    f"cache data missing for {missing_paths} on run {step}"
                )
    ...
    def find_missing_cache_files(
        ...
        for id, path in modules.items():
            ...
            if meta_pair is None:
                missing[id] = path
            elif not build.validate_meta(
                meta_pair[0], id, path, ignore_errors, manager
            ):
                missing[id] = path
