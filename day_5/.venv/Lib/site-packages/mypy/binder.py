...
class FrameContext:
    ...
    def __enter__(self) -> Frame:
        ...
        if self.continue_frame:
            self.binder.continue_frames.append(
                len(self.binder.frames) - self.continue_frame
            )
        if self.try_frame:
    ...
    def update_from_options(self, frames: list[Frame]) -> bool:
        ...
        for key in keys:
            ...
            old_semantics = (
                not self.bind_all
                or var is None
                or not var.is_inferred
                and not var.is_argument
            )
            ...
            if (
                ...
                if (
                    ...
                    or extract_var_from_literal_hash(key) is None
                ):
    ...
    def assign_type(
        ...
        if isinstance(p_type, AnyType):
            ...
            if (
                ...
                if any(
                    isinstance(get_proper_type(item), NoneType) for item in all_items
                ):
                    ...
                    isinstance(get_proper_type(item), AnyType) for item in all_items
                ):
