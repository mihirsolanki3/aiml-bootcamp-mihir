...
class IterationDependentErrors:
        ...
        for candidate in set(chain(*self.uselessness_errors)):
            if all(
                ...
                for errors, lines in zip(
                    self.uselessness_errors, self.unreachable_lines
                )
            ):
    ...
    def yield_nonoverlapping_types(
        ...
        persistent_nonoverlaps: dict[
            NonOverlapErrorInfo, tuple[list[Type], list[Type]]
        ] = defaultdict(lambda: ([], []))
    ...
    def on_error(self, file: str, info: ErrorInfo) -> bool:
        ...
        if info.code in (codes.UNREACHABLE, codes.REDUNDANT_EXPR, codes.REDUNDANT_CAST):
            ...
            if info.code == codes.UNREACHABLE:
                iter_errors.unreachable_lines[-1].update(
                    range(info.line, info.end_line + 1)
                )
            return True
...
class Errors:
    ...
    def add_error_info(self, info: ErrorInfo, *, file: str | None = None) -> None:
        ...
        if (
            ...
            self.note_for_info(
                file, info, message, info.code, only_once=True, priority=20
            )
    ...
    def generate_unused_ignore_errors(
        ...
        for line, ignored_codes in ignored_lines.items():
            ...
            unused_ignored_codes = [
                c for c in ignored_codes if c not in used_ignored_codes
            ]
            # `ignore` is used
    ...
    def generate_ignore_without_code_errors(
        ...
        for line, ignored_codes in self.ignored_lines[file].items():
            ...
            self.report_simple_error(
                file, line, message, code=codes.IGNORE_WITHOUT_CODE
            )
            ...
            if path not in self.flushed_files:
                ...
                msgs.extend(
                    self.format_messages(
                        path, error_tuples, formatter=self.error_formatter
                    )
                )
        return msgs
        ...
        for e in errors:
            ...
            if not self.options.show_error_context:
                ...
                while i >= 0:
                    ...
                    result.append(
                        (None, -1, -1, -1, -1, "note", fmt.format(path, line), None)
                    )
                    i -= 1
            ...
            if not self.options.show_error_context:
                ...
                if function is None:
                    if type is None:
                        result.append(
                            (file, -1, -1, -1, -1, "note", "At top level:", None)
                        )
                    else:
                        result.append(
                            (file, -1, -1, -1, -1, "note", f'In class "{type}":', None)
                        )
                else:
    ...
    def sort_messages(self, errors: list[ErrorInfo]) -> list[ErrorInfo]:
        ...
        while i < len(errors):
            ...
            while (
                i + 1 < len(errors) and errors[i + 1].import_ctx == errors[i].import_ctx
            ):
                i += 1
            i += 1
