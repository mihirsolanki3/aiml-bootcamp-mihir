...
NOT_IMPLEMENTED_TYPE_NAMES: Final = (
    ...
    def is_recursive(self) -> bool:
        ...
        *definition* is recursive. The property value thus can be cached on the
        ...
        if is_recursive is None:
            is_recursive = self.alias in self.alias.target.accept(
...
class UninhabitedType(ProperType):
    ...
    def __init__(
        self, line: int = -1, column: int = -1, *, ambiguous: bool = False
    ) -> None:
        super().__init__(line, column)
...
class ExtraAttrs:
    ...
    def read(cls, data: ReadBuffer) -> ExtraAttrs:
        ret = ExtraAttrs(
            read_type_map(data), set(read_str_list(data)), read_str_opt(data)
        )
        assert read_tag(data) == END_TAG
        return ret
...
class FormalArgument:
    def __init__(
        self, name: str | None, pos: int | None, typ: Type, required: bool
    ) -> None:
        self.name = name
        self.pos = pos
        self.typ = typ
...
class CallableType(FunctionLike):
    ...
    def copy_modified(
        ...
        modified = CallableType(
            ...
            instance_type=(
                instance_type if instance_type is not _dummy else self.instance_type
            ),
        )
...
class TypedDictType(ProperType):
    ...
    ) -> Iterable[tuple[str, TypedDictItem, TypedDictItem]]:
        left = self
    ...
    def visit_callable_type(self, t: CallableType, /) -> str:
        ...
        for i in range(len(t.arg_types) - num_skip):
            ...
            if not name and not self.options.reveal_verbose_types:
                ...
                if t.arg_kinds[i] == ARG_STAR and isinstance(
                    t.arg_types[i], UnpackType
                ):
                    name = "args"
...
class CollectAliasesVisitor(TypeQuery[list[mypy.nodes.TypeAlias]]):
    ...
    def strategy(
        self, items: list[list[mypy.nodes.TypeAlias]]
    ) -> list[mypy.nodes.TypeAlias]:
        out = []
        for item in items:
            out.extend(item)
        return out
...
def flatten_nested_tuples(
    types: Iterable[Type], handle_recursive: bool = True
) -> list[Type]:
