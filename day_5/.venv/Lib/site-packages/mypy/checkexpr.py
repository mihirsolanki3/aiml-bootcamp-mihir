...
ArgChecker: _TypeAlias = Callable[
    [Type, Type, ArgKind, Type, int, int, CallableType, Type | None, Context, Context],
    None,
]
...
class ExpressionChecker(ExpressionVisitor[Type], ExpressionCheckerSharedApi):
    ...
    def analyze_ref_expr(self, e: RefExpr, lvalue: bool = False) -> Type:
        ...
        if isinstance(node, Var):
            ...
            node, (FuncDef, TypeInfo, TypeAlias, MypyFile, TypeVarLikeExpr)
        ):
    ...
    def analyze_static_reference(
        ...
        *,
        ...
        if isinstance(node, (Var, Decorator, OverloadedFuncDef)):
            ...
            return (
                self.module_type(node)
                if include_modules
                else AnyType(TypeOfAny.special_form)
            )
        ...
        if self.chk.options.extra_checks and last_open_star_found is not None:
            ...
            if absent_keys:
                ...
                self.msg.non_required_keys_absent_with_star(
                    absent_keys, last_open_star_found
                )
    ...
    def replace_type_type_callee_ret_type(self, callee: Type, ret_type: Type) -> Type:
        ...
        if isinstance(callee, Overloaded):
            return Overloaded(
                [c.copy_modified(ret_type=ret_type) for c in callee.items]
            )
    ...
    def infer_function_type_arguments_using_context(
        ...
        if (
            ...
            and all(
                isinstance(get_proper_type(u), TypeVarType) for u in proper_ret.items
            )
        ):
    ...
    def dangerous_comparison(
        ...
        *,
        ...
        if (
            ...
            or custom_special_method(right, "__eq__")
    ...
    def check_op_reversible(
        ...
        if op_name in operators.op_methods_that_shortcut and is_same_type(
            ...
            (
                ...
                not (
                    isinstance(left_type, Instance) and isinstance(right_type, Instance)
                )
                or (
    ...
    def visit_type_form_expr(self, expr: TypeFormExpr) -> Type:
        ...
        return TypeType.make_normalized(
            typ, line=typ.line, column=typ.column, is_type_form=True
        )
    ...
    def split_for_callable(
        ...
        if (
            ...
            or t.type_object().fullname == "builtins.tuple"
        ):
            return list(args)
        info = t.type_object()
    ...
    def visit_dict_expr(self, e: DictExpr) -> Type:
        ...
        typeddict_contexts, exhaustive = self.find_typeddict_context(
            self.type_context[-1], e
        )
    ...
    def find_typeddict_context(
        ...
        if isinstance(context, TypedDictType):
            ...
            for item in context.items:
                item_contexts, item_exhaustive = self.find_typeddict_context(
                    item, dict_expr
                )
    ...
    def accept(
        ...
        try:
            ...
            if allow_none_return and isinstance(node, CallExpr):
                ...
                and (node_as_type := self.try_parse_as_type_expression(node))
                is not None
            ):
            ...
            # TODO: consider using cache for more expression kinds.
                ...
                if (node, type_context) in self.expr_cache:
                    binder_version, typ, messages, type_map = self.expr_cache[
                        (node, type_context)
                    ]
                ...
                typ = node.accept(
                    self
                )  # r-value type, when interpreted as a value expression
    ...
    def accept_maybe_cache(
        ...
        if (
            ...
            self.expr_cache[(node, type_context)] = (
                binder_version,
                typ,
                messages,
                type_map,
    ...
    def visit_type_var_expr(self, e: TypeVarExpr) -> Type:
        ...
        if not (
            ...
            if e.values and not any(
                is_same_type(p_default, value) for value in e.values
            ):
    ...
    def try_parse_as_type_expression(self, maybe_type_expr: Expression) -> Type | None:
        ...
        sym_for_name = {
            e.name: SymbolTableNode(UNBOUND_IMPORTED, e.node) for e in name_exprs
        } | {
