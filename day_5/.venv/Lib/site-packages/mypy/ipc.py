    ...
    def __init__(self, name: str, timeout: float | None = None) -> None:
        if sys.platform == "win32":
            name = r"\\.\pipe\{}-{}.pipe".format(
                name, urlsafe_b64encode(os.urandom(6)).decode()
            )
        else:
            name = f"{name}.sock"
        super().__init__(name, timeout)
        ...
        if sys.platform == "win32":
            ...
            try:
                ...
                self.connection.setsockopt(
                    socket.SOL_SOCKET, socket.SO_RCVBUF, MAX_READ
                )
                self.connection.setsockopt(
                    socket.SOL_SOCKET, socket.SO_SNDBUF, MAX_READ
                )
...
def ready_to_read(conns: Sequence[IPCBase], timeout: float | None = None) -> list[int]:
    ...
    if sys.platform == "win32":
        ...
        if not ready and events:
            timeout_ms = (
                int(timeout * 1000) if timeout is not None else _winapi.INFINITE
            )
