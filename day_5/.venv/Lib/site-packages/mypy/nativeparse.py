...
def parse_to_binary_ast(
    ...
    bytes, list[ParseError], TypeIgnores, bytes, bool, bool, str, list[tuple[int, str]]
]:
...
def read_type_alias_stmt(state: State, data: ReadBuffer) -> TypeAliasStmt:
    ...
    assert isinstance(
        name, NameExpr
    ), f"Expected NameExpr for type alias name, got {type(name)}"
    ...
    if n_type_params > 0:
        ...
        for _ in range(n_type_params):
            ...
            type_params.append(
                TypeParam(param_name, kind, upper_bound, values, default)
            )
    else:
        type_params = []
...
def read_type(state: State, data: ReadBuffer) -> Type:
    ...
    if tag == types.UNBOUND_TYPE:
        ...
        if type_name == "builtins.bool":
            ...
            assert (
                tag == LITERAL_NONE
            ), f"Expected LITERAL_NONE for invalid type, got {tag}"
            value = None
...
def read_pattern(state: State, data: ReadBuffer) -> Pattern:
    ...
    if tag == nodes.AS_PATTERN:
        ...
        class_pattern = ClassPattern(
            class_ref, positionals, keyword_keys, keyword_values
        )
...
def read_expression(state: State, data: ReadBuffer) -> Expression:
    ...
    if tag == nodes.NAME_EXPR:
        ...
        expr = DictionaryComprehension(
            key, value, indices, sequences, condlists, is_async
        )
        read_loc(data, expr)
        expect_end_tag(data)
        return expr
...
def check_ifstmt_for_overloads(
    ...
    if len(stmt.else_body.body) == 1:
        ...
        if (
            ...
            and check_ifstmt_for_overloads(
...
def get_executable_if_block_with_overloads(
    ...
    if stmt.body[0].is_unreachable is True:
        ...
        if isinstance(stmt.else_body.body[0], IfStmt):
            return get_executable_if_block_with_overloads(
                stmt.else_body.body[0], options
            )
