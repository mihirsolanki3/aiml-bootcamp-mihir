...
class PatternChecker(PatternVisitor[PatternType]):
    ...
    def visit_value_pattern(self, o: ValuePattern) -> PatternType:
        ...
        ok_type = (
            ok_map.get(node, current_type) if ok_map is not None else UninhabitedType()
        )
        rest_type = (
            rest_map.get(node, current_type)
    ...
    def visit_sequence_pattern(self, o: SequencePattern) -> PatternType:
        ...
        if isinstance(current_type, UnionType):
            ...
            return PatternType(
                ...
                {
                    expr: make_simplified_union(types)
                    for expr, types in captures_list.items()
                },
            )
        ...
        elif isinstance(current_type, Instance) and self.chk.type_is_iterable(
            current_type
        ):
    ...
    def get_class_pattern_type_ranges(
        self, typ: Type, o: ClassPattern
    ) -> list[TypeRange] | None:
        p_typ = get_proper_type(typ)
    ...
    def construct_sequence_child(self, outer_type: Type, inner_type: Type) -> Type:
        ...
        if isinstance(proper_type, TypeVarType):
            new_bound = self.construct_sequence_child(
                proper_type.upper_bound, inner_type
            )
