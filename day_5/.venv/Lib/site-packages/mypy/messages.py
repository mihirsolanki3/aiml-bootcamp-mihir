...
class MessageBuilder:
        ...
        *,
        ...
        if origin_context is not None:
            origin_span = itertools.chain(
                origin_span, self.span_from_context(origin_context)
            )
    ...
    def fail(
        ...
        *,
        ...
        return self.report(
            msg, context, "error", code=code, origin_context=origin_context
        )
    ...
    def incompatible_argument_note(
        ...
        if isinstance(callee_type, CallableType) and isinstance(
            ...
            if call:
                self.note_call(
                    original_caller_type, call, context, code=parent_error.code
                )
        if isinstance(callee_type, Instance) and callee_type.type.is_protocol:
        ...
        if unexpected_kwargs:
            ...
            if kwargs_without_suggestions:
                if len(kwargs_without_suggestions) == 1:
                    ...
                    quoted_names = ", ".join(
                        f'"{n}"' for n in kwargs_without_suggestions
                    )
                    self.fail(
    ...
    def signature_incompatible_with_supertype(
        ...
        *,
        ...
        self.note(
            "Superclass:", context, offset=ALIGN_OFFSET + OFFSET, parent_error=error
        )
        ...
        self.note(
            "Subclass:", context, offset=ALIGN_OFFSET + OFFSET, parent_error=error
        )
        ...
        if isinstance(tp, CallableType):
            if add_class_or_static_decorator:
                ...
                if decorator is not None:
                    self.note(
                        decorator, context, offset=offset, parent_error=parent_error
                    )
            self.note(
    ...
    def invalid_keyword_var_arg(
        ...
        if isinstance(typ, Instance) and is_mapping:
            self.fail(
                "Argument after ** must have string keys", context, code=codes.ARG_TYPE
            )
        else:
            self.fail(
    ...
    def incompatible_conditional_function_def(
        ...
        if isinstance(old_type, (CallableType, Overloaded)) and isinstance(
            ...
            self.pretty_callable_or_overload(
                old_type, defn, offset=4, parent_error=error
            )
            self.note("Redefinition:", defn)
            self.pretty_callable_or_overload(
                new_type, defn, offset=4, parent_error=error
            )
    ...
    def overloaded_signatures_param_specific(
        self, index: int, context: Context
    ) -> None:
        self.fail(
            (
    ...
    def report_protocol_problems(
        ...
        *,
        ...
        if conflict_types and (
            ...
            self.print_more(
                conflict_types, context, OFFSET, MAX_ITEMS, code=parent_error.code
            )
        ...
        self.print_more(
            conflict_flags, context, OFFSET, MAX_ITEMS, code=parent_error.code
        )
...
def should_format_arg_as_type(
    arg_kind: ArgKind, arg_name: str | None, verbosity: int
) -> bool:
    """
...
def get_first_arg(tp: CallableType) -> str | None:
    definition = get_func_def(tp)
    if (
        not isinstance(definition, FuncDef)
        or not definition.info
        or definition.is_static
    ):
        return None
