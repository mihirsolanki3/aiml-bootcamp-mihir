...
class TypeAnalyser(SyntheticTypeVisitor[Type], TypeAnalyzerPluginInterface):
    ...
    def visit_unbound_type_nonoptional(
        ...
        if sym is not None:
            ...
            if isinstance(node, TypeAlias):
                ...
                if (
                    self.analyzing_tvar_def
                    and used_default
                    and isinstance(res, TypeAliasType)
                ):
    ...
    def pack_paramspec_args(
        self, an_args: Sequence[Type], empty_tuple_index: bool
    ) -> list[Type]:
    ...
    def try_analyze_special_unbound_type(
        ...
        if fullname == "builtins.None":
            ...
            return TypeType.make_normalized(
                item, line=t.line, column=t.column, is_type_form=True
            )
            ...
            if self.defining_alias:
                self.fail(
                    "ClassVar[...] can't be used inside a type alias",
                    t,
                    code=codes.VALID_TYPE,
                )
            if len(t.args) == 0:
            ...
            if self.api.type.self_type is not None:
                if (
                    self.api.type.is_final
                    or self.api.type.is_enum
                    and self.api.type.enum_members
                ):
    ...
    def analyze_type_with_type_info(
        ...
        if len(info.type_vars) == 1 and info.has_param_spec_type:
            instance.args = tuple(
                self.pack_paramspec_args(instance.args, empty_tuple_index)
            )
    ...
    def visit_callable_type(
        ...
        with self.tvar_scope_frame(namespace=namespace):
            ...
            type_guard = (
                self.anal_type_guard(t.ret_type)
    ...
    def bind_function_type_variables(
        ...
        if fun_type.variables:
            ...
            for var in fun_type.variables:
                ...
                binding = self.tvar_scope.bind_new(
                    var.name, var_expr, self.fail_func, fun_type
                )
                defs.append(binding)
        ...
        for name, tvar in typevars:
            if not self.tvar_scope.allow_binding(tvar.fullname):
                err_msg = message_registry.TYPE_VAR_REDECLARED_IN_NESTED_CLASS.format(
                    name
                )
...
def get_omitted_any(
    ...
    if disallow_any:
        ...
        type_str = (
            typ.name if isinstance(typ, UnboundType) else format_type_bare(typ, options)
        )
