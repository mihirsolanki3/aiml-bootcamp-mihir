...
def _infer_constraints(
    ...
    return template.accept(
        ConstraintBuilderVisitor(actual, direction, skip_neg_op, erase_types)
    )
        ...
        if isinstance(self.actual, CallableType):
            ...
            if isinstance(item, Instance):
                ...
                if isinstance(constructor, CallableType):
                    ...
                    return self.infer_against_overloaded(
                        constructor, template, ret_only=True
                    )
            return infer_constraints(
            ...
            if unpack_index is not None:
                if is_varlength_tuple:
                    ...
                    if isinstance(unpacked_type, TypeVarTupleType):
                        res = [
                            Constraint(
                                type_var=unpacked_type, op=self.direction, target=mapped
                            )
                        ]
                    else:
                        assert (
                    ...
                    for i, ti in enumerate(template.items):
                        ...
                        res.extend(
                            infer_constraints(ti, mapped.args[0], self.direction)
                        )
                    return res
                else:
