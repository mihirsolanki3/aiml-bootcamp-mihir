...
def type_object_type_from_function(
    ...
    if isinstance(signature, CallableType):
        ...
        for item, orig_self in zip(signature.items, orig_self_types):
            items.append(
                class_callable(
                    item, info, def_info, fallback, special_sig, is_new, orig_self
                )
            )
    ...
    if isinstance(t, TypeType):
        if isinstance(t.item, TypeVarType):
            return TypeType.make_normalized(
                t.item.upper_bound, is_type_form=t.is_type_form
            )
    return t
        ...
        if extra_attrs_set is not None:
            ...
            if len(extra_attrs_set) > 1:  # This case is too tricky to handle.
                ...
                for item in items:
                    ...
                    if (
                        instance
                        and instance.type == fallback.type
                        and not instance.extra_attrs
                    ):
                        erase_extra = True
                        break
            if erase_extra:
...
def false_only(t: Type) -> ProperType:
    ...
    if not t.can_be_false:
        ...
        if ret_type:
            ...
            isinstance(t, LiteralType) and t.is_enum_literal() and state.strict_optional
        ):
            return UninhabitedType(line=t.line)
...
def is_singleton_identity_type(typ: ProperType) -> bool:
    ...
    if (
        ...
        and isinstance(typ.item, Instance)
        and typ.item.type.is_final
    ):
        return True
    if (
        isinstance(typ, FunctionLike)
        and typ.is_type_obj()
        and typ.type_object().is_final
    ):
        return True
    return False
...
def can_have_shared_disjoint_base(instances: list[Instance]) -> bool:
    ...
    disjoint_bases = [
        ...
        if (base := _get_disjoint_base_of(instance)) is not None
    ]
