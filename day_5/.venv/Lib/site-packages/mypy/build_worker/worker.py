...
def serve(server: IPCServer, ctx: ServerContext) -> None:
    ...
    while True:
        ...
        try:
            load_states(
                mod_ids, graph, manager, scc_message.import_errors, scc_message.mod_data
            )
            results = []
            for scc in sccs:
            ...
            message = SccResponseMessage(
                scc_ids=scc_ids, is_interface=True, blocker=blocker
            )
            ...
            message = SccResponseMessage(
                scc_ids=scc_ids, is_interface=True, result=mod_results
            )
            timed_send(manager, server, message)
            try:
                ...
                for id, meta_file in zip(stale, meta_files):
                    result |= process_stale_scc_implementation(
                        graph, [id], manager, [meta_file]
                    )
                # Both phases write cache, so we should commit here as well.
                manager.commit()
            except CompileError as blocker:
                message = SccResponseMessage(
                    scc_ids=scc_ids, is_interface=False, blocker=blocker
                )
            else:
                message = SccResponseMessage(
                    scc_ids=scc_ids, is_interface=False, result=result
                )
            timed_send(manager, server, message)
        manager.add_stats(
            total_process_stale_time=time.time() - t0, stale_sccs_processed=1
        )


def timed_send(
    manager: BuildManager, server: IPCServer, message: SccResponseMessage
) -> None:
    t0 = time.time()
    send(server, message)
...
def load_states(
    ...
    for id in mod_ids:
        ...
        state.imports_ignored = {
            line: codes
            for line, codes in state.tree.ignored_lines.items()
