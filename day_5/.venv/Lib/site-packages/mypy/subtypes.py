...
def _is_subtype(
    ...
    if (
        isinstance(right, TypeVarType)
        and right.values
        and not isinstance(left, TypeVarType)
    ):
        if proper_subtype:
            if all(
            ...
            is_subtype(orig_left, v, subtype_context=subtype_context)
            for v in right.values
        ):
            return True
        ...
        if isinstance(right, CallableType):
            ...
            if right.type.is_protocol and left.is_type_obj():
                ...
                if isinstance(instance_type, Instance) and is_protocol_implementation(
                    instance_type,
                    right,
                    proper_subtype=self.proper_subtype,
                    class_obj=True,
                ):
                    return True
            ...
            return left.is_type_obj() and self._is_subtype(
                left.get_instance_type(), right.item
            )
        else:
            return False
        ...
        if left_unpack_index is None:
            ...
            if not isinstance(left_unpacked, Instance):
                ...
                if self.is_top_type(right_item):
                    right_prefix_types, middle, right_suffix_types = (
                        split_with_prefix_and_suffix(
            ...
            if isinstance(right, CallableType):
                ...
                if isinstance(item, Instance):
                    ...
                    if isinstance(constructor, CallableType):
                        ...
                        return all(
                            self._is_subtype(c.ret_type, right.ret_type)
                            for c in constructor.items
                        )
                # This is unsound, we don't check the __init__ signature.
        ...
        for member in right.type.protocol_members:
            ...
            if IS_SETTABLE in superflags:
                ...
                if IS_EXPLICIT_SETTER in superflags:
                    supertype = find_member(
                        member, right, original_left, is_lvalue=True
                    )
...
def get_protocol_member(
    ...
    subtype = find_member(
        member, left, original_left, class_obj=class_obj, is_lvalue=is_lvalue
    )
    ...
    *,
    ...
    with type_checker.msg.filter_errors(filter_deprecated=True):
        if class_obj:
            ...
            return analyze_class_attribute_access(
                itype, name, mx, mcs_fallback=fallback
            )
        else:
...
def restrict_subtype_away(
    t: Type, s: Type, *, consider_runtime_isinstance: bool = True
) -> Type:
    ...
    if isinstance(p_t, UnionType):
        ...
        return UnionType.make_union(
            [
                ...
                if not isinstance(get_proper_type(item), UninhabitedType)
            ]
        )
