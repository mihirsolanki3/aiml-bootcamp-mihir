...
class FileRawData:
    ...
    def read(cls, data: ReadBuffer) -> FileRawData:
        ...
        assert read_tag(data) == LIST_GEN
        mypy_comments = [
            (read_int(data), read_str(data)) for _ in range(read_int_bare(data))
        ]
        return FileRawData(
            defs,
            imports,
...
class OverloadedFuncDef(FuncBase, SymbolNode, Statement):
    ...
    def write(self, data: WriteBuffer) -> None:
        ...
        write_flags(
            data, [self.is_property, self.is_class, self.is_static, self.is_final]
        )
    ...
    def read(cls, data: ReadBuffer) -> OverloadedFuncDef:
        assert read_tag(data) == LIST_GEN
        res = OverloadedFuncDef(
            [read_overload_part(data) for _ in range(read_int_bare(data))]
        )
        ...
        if tag != LITERAL_NONE:
            ...
            if len(res.items) > 0:
                res.set_line(res.impl.line)
        res.is_property, res.is_class, res.is_static, res.is_final = read_flags(
            data, num_flags=4
        )
...
class Var(SymbolNode):
    ...
    def serialize(self) -> JsonDict:
        ...
        data: JsonDict = {
            ...
            "type": None if self.type is None else self.type.serialize(),
            "setter_type": (
                None if self.setter_type is None else self.setter_type.serialize()
            ),
...
class TypeInfo(SymbolNode):
    ...
    def explain_metaclass_conflict(self) -> str | None:
        ...
        if declared is None:
            ...
            resolution_steps = [
                f'"{declared.type.fullname}" (metaclass of "{self.fullname}")'
            ]
        for super_class in self.mro[1:]:
            super_meta = super_class.declared_metaclass
            if super_meta is None or super_meta.type is None:
            ...
            # metaclass conflict
            conflict = (
                f'"{super_meta.type.fullname}" (metaclass of "{super_class.fullname}")'
            )
        ...
        if self.unfixed:
            ...
            if self.cross_ref is not None:
                ...
                if self._node is None:
                    self._node = read_symbol(
                        ReadBuffer(self._node_bytes), self._node_tag
                    )
                    self._node_bytes = b""
                node = self._node
        ...
        if node and node.fullname == fullname:
            ...
            if impl_only:
                if not isinstance(node, (FuncDef, OverloadedFuncDef, Decorator)):
                    ...
                    if (
                        symnode.plugin_generated
                        and info
                        and node not in info.defn.defs.body
                    ):
                        yield_node = False
            if (
                isinstance(node, (FuncDef, OverloadedFuncDef, Decorator))
                and "@" in fullname
            ):
                yield_node = False
            if yield_node:
