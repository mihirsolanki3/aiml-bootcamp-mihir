...
LIBRT_MODULES = [
    ...
    ModDesc(
        "librt.random", ["random/librt_random.c"], ["random/librt_random.h"], ["random"]
    ),
    ModDesc(
        "librt.threading",
    ...
    try:
        ...
        if options.mypyc_annotation_file:
            generate_annotated_html(
                options.mypyc_annotation_file, result, modules, mapper
            )

        # Collect SourceDep dependencies
        source_deps = sorted(
            emitmodule.collect_source_dependencies(modules), key=lambda d: d.path
        )
    ...
    if separate is True:
        ...
        for files, name in separate:
            ...
            group_sources = [
                ...
                and os.path.normpath(src.path) in normalized_files
            ]
...
def mypyc_build(
    ...
    *,
    ...
    if compiler_options.group_name is not None:
        assert (
            len(groups) == 1
        ), "If using custom group_name, only one group is expected"
    ...
    if install_librt:
        ...
        for mod, file_names, addit_files, includes in LIBRT_MODULES:
            for file_name in file_names + addit_files:
                ...
                with open(
                    os.path.join(include_dir(), file_name), encoding="utf-8"
                ) as f:
                    write_file(rt_file, f.read())
            extensions.append(
                get_extension()(
                    ...
                    sources=[
                        ...
                        for file in file_names + RUNTIME_C_FILES
                    ],
