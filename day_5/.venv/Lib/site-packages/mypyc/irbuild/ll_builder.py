...
class LowLevelIRBuilder:
    ...
    def flush_keep_alives(
        self, line: int, *, scope: int = KEEP_ALIVE_SHORT_LIVED
    ) -> None:
        if any(entry.scope == scope for entry in self.keep_alives):
            self.add(
                KeepAlive(
                    [entry.value for entry in self.keep_alives if entry.scope == scope],
                    line,
                )
            )
            self.keep_alives = [
                entry for entry in self.keep_alives if entry.scope != scope
            ]
    ...
    def flush_keep_alives_since(self, line: int, checkpoint: int) -> None:
        ...
        new_keep_alives = [
            entry for entry in self.keep_alives if entry.seq >= checkpoint
        ]
        if new_keep_alives:
    ...
    def add_keep_alive(self, value: Value, scope: int) -> None:
        self.keep_alives.append(
            PendingKeepAlive(value, scope, self.next_keep_alive_seq)
        )
    ...
    def unbox_or_cast(
        ...
        *,
        ...
        if target_type.is_unboxed:
            ...
            return self.add(
                Cast(src, target_type, line, borrow=can_borrow, unchecked=unchecked)
            )
        ...
        if (src_type.is_unboxed and target_type.is_unboxed) and not is_runtime_subtype(
            ...
            if (
                ...
                if src_type.size == target_type.size:
                    ...
                    return self.add(
                        Extend(src, target_type, target_type.is_signed, line)
                    )
            elif is_int64_rprimitive(src_type) and is_c_py_ssize_t_rprimitive(
                target_type
            ):
            ...
            elif is_bool_or_bit_rprimitive(src_type) and is_fixed_width_rtype(
                target_type
            ):
    ...
    def coerce_fixed_width_to_int(self, src: Value, line: int) -> Value:
        ...
        c1 = self.add(
            ComparisonOp(src, Integer(MAX_SHORT_INT, src_type), ComparisonOp.SLE, line)
        )
        self.add(Branch(c1, fast, slow, Branch.BOOL))

        self.activate_block(fast)
        c2 = self.add(
            ComparisonOp(src, Integer(MIN_SHORT_INT, src_type), ComparisonOp.SGE, line)
        )
            ...
            if kind == ARG_STAR:
                if star_result is None:
                    ...
                    if len(args) == 1:
                        ...
                        if is_tuple_rprimitive(value.type) or isinstance(
                            value.type, RTuple
                        ):
                            star_result = value
                        elif is_list_rprimitive(value.type):
                            star_result = self.primitive_op(
                                list_tuple_op, [value], line
                            )
                        else:
                            star_result = self.primitive_op(
                                sequence_tuple_op, [value], line
                            )
    ...
    def py_call(
        ...
        if kw_args_dict is None:
            return self.call_c(
                py_call_with_posargs_op, [function, pos_args_tuple], line
            )
        ...
        if arg_kinds is None or all(
            ...
            if arg_values:
                ...
                arg_ptr = self.setup_rarray(
                    object_rprimitive, coerced_args, line, object_ptr=True
                )
            else:
    ...
    def gen_method_call(
        ...
        if (
            ...
            and (
                base.type.class_ir.is_ext_class
                or base.type.class_ir.has_method(fast_name)
            )
        ...
        if op == "==":
            ...
            if literal_fastpath:
                ...
                literal_length = Integer(
                    len(literal_string), c_pyssize_t_rprimitive, line
                )
                return self.primitive_op(
                    str_eq_literal, [lhs, rhs, literal_length], line
                )
    ...
    def unary_not(self, value: Value, line: int, *, likely_bool: bool = False) -> Value:
        ...
        if is_optional_type(typ):
            ...
            if (
                ...
                cmp = self.add(
                    ComparisonOp(value, self.none_object(), ComparisonOp.EQ, line)
                )
                ...
                val = self.unary_not(
                    self.unbox_or_cast(
                        value, value_typ, line, can_borrow=True, unchecked=True
                    ),
                    line,
                )
        ...
        if likely_bool and is_object_rprimitive(typ):
            ...
            out = BasicBlock()
            cmp = self.add(
                ComparisonOp(value, self.true_object(), ComparisonOp.EQ, line)
            )
            ...
            cmp = self.add(
                ComparisonOp(value, self.false_object(), ComparisonOp.EQ, line)
            )
    ...
    def primitive_op(
        ...
        if desc.type_params is not None:
            assert (
                type_args is not None
            ), "Generic primitive op requires explicit type arguments"
            assert len(type_args) == len(desc.type_params)
            arg_types = [
                expand_rtype(arg_type, type_args) for arg_type in desc.arg_types
            ]
    ...
    def builtin_len(self, val: Value, line: int, use_pyssize_t: bool = False) -> Value:
        ...
        if isinstance(typ, RInstance):
            ...
            return self.int_op(
                short_int_rprimitive, len_value, count, IntOp.LEFT_SHIFT, line
            )
    ...
    def translate_eq_cmp(
        ...
        if lopt is not None:
            if (
                ropt is not None
                and is_same_type(lopt, ropt)
                and self._never_equal_to_none(lopt)
            ):
                fast_opt_eq = True
    ...
    def _translate_fast_optional_eq_cmp(
        ...
        if not isinstance(rreg.type, RUnion):
            ...
            eq = self.translate_eq_cmp(
                self.unbox_or_cast(
                    lreg, value_typ, line, can_borrow=True, unchecked=True
                ),
                rreg,
                expr_op,
                line,
            ...
            cmp = self.add(
                ComparisonOp(rreg, self.none_object(), ComparisonOp.EQ, line)
            )
            ...
            eq = self.translate_eq_cmp(
                ...
                self.unbox_or_cast(
                    rreg, value_typ, line, can_borrow=True, unchecked=True
                ),
                expr_op,
                line,
            )
