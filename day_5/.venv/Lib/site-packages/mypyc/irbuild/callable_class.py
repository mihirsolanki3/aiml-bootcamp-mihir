...
def setup_callable_class(builder: IRBuilder) -> None:
    ...
    # class.
    callable_class_ir = ClassIR(
        name, builder.module_name, is_generated=True, is_final_class=True
    )
...
def add_coroutine_properties(
    ...
    for name, primitive in properties.items():
        with builder.enter_method(
            callable_class_ir, name, object_rprimitive, internal=True
        ):
            func = get_func_wrapper()
            val = builder.primitive_op(
                primitive, [func, Integer(0, c_pointer_rprimitive)], line
            )
