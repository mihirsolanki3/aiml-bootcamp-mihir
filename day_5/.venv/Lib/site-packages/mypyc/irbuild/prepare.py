...
def create_generator_class_for_func(
    ...
    cir = ClassIR(
        name, module_name, is_generated=True, is_final_class=class_name is None
    )
...
def _check_deletable_declarations(
    path: str, cdef: ClassDef, ir: ClassIR, errors: Errors
) -> None:
...
def prepare_class_def(
    ...
    if free_list_len is not None:
        ...
        if free_list_len == 1:
            ...
            errors.error(
                f'Unsupported value for "free_list_len": {free_list_len}', path, line
            )
...
def prepare_init_method(
    ...
    if (
        new_symbol
        and new_symbol.fullname
        and not new_symbol.fullname.startswith("builtins.")
    ):
            ...
            for i, d in enumerate(decorators_to_store):
                ...
                if impl is not None:
                    if self.func_stack_depth > 0:
                        self.errors.error(
                            "Registering nested functions not supported",
                            self.current_path,
                            d.line,
                        )
