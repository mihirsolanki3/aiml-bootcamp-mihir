...
def translate_list_comprehension(builder: IRBuilder, gen: GeneratorExpr) -> Value:
    ...
    def gen_inner_stmts() -> None:
        ...
        builder.primitive_op(
            list_append_op, [builder.read(list_ops, gen.line), e], gen.line
        )
...
def translate_vec_comprehension(
    ...
    *,
    ...
    builder.assign(
        vec,
        vec_create(builder.builder, vec_type, 0, gen.line, capacity=capacity),
        gen.line,
    )
    loop_params = list(zip(gen.indices, gen.sequences, gen.condlists, gen.is_async))
...
class ForAsyncIterable(ForGenerator):
    ...
    def gen_condition(self) -> None:
        ...
        def except_match() -> Value:
            return builder.add(
                LoadGlobal(
                    stop_async_iteration_op.type, stop_async_iteration_op.src, line
                )
            )

        def try_body() -> None:
            awaitable = builder.call_c(
                anext_op, [builder.read(self.iter_target, line)], line
            )
...
class ForSequence(ForGenerator):
    ...
    def init(
        ...
        assert is_sequence_rprimitive(expr_reg.type) or isinstance(
            expr_reg.type, RVec
        ), (
            expr_reg,
            expr_reg.type,
        ...
        if is_immutable_rprimitive(expr_reg.type):
            ...
            self.length_reg = builder.maybe_spill(
                self.length or self.load_len(self.expr_target)
            )
        else:
...
def get_expr_length(builder: IRBuilder, expr: Expression) -> int | None:
    ...
    if isinstance(folded, (str, bytes)):
        ...
        stars = [
            get_expr_length(builder, i) for i in expr.items if isinstance(i, StarExpr)
        ]
...
def get_expr_length_value(
    ...
    return Integer(
        length, c_pyssize_t_rprimitive if use_pyssize_t else short_int_rprimitive
    )
