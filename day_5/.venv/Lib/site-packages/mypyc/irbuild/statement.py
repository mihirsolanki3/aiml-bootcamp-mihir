...
def check_unsupported_cls_assignment(builder: IRBuilder, stmt: AssignmentStmt) -> None:
    ...
    for lvalue in lvalues:
        if isinstance(lvalue, NameExpr) and lvalue.name == cls_arg:
            ...
            builder.error(
                f'Assignment to argument "{cls_arg}" in "__new__" method unsupported',
                stmt.line,
            )
        ...
        for mod_id, as_name in imp.ids:
            flat_list.append(
                (
                    ...
                    as_name,
                    imp.line,
                    builder.is_native_module(mod_id)
                    and builder.is_same_group_module(mod_id),
                )
            )
    result = []
    i = 0
...
def transform_non_native_import_group(
    ...
    import_line_ptr = builder.builder.setup_rarray(
        c_pyssize_t_rprimitive, mod_lines, first_line
    )
    builder.call_c(
        import_many_op,
        [
...
def transform_import_from(builder: IRBuilder, node: ImportFrom) -> None:
    ...
    transform_import_from_buckets(
        builder, id, names, as_names, node.line, parent_is_native
    )
...
def group_consecutive(items: list[tuple[int, str, str]]) -> list[ImportFromBucket]:
    ...
    while i < len(items):
        ...
        result.append(
            ImportFromBucket(
                kind, [t[1] for t in items[i0:i]], [t[2] for t in items[i0:i]]
            )
        )
    return result
...
def transform_import_from_buckets(
    ...
    buckets = classify_import_from(
        builder, module_id, names, as_names, parent_is_native
    )
    module = None
    for bucket in buckets:
        if bucket.kind == IMPORT_NATIVE_SUBMODULE:
            ...
            names_literal = builder.add(
                LoadLiteral(tuple(bucket.names), object_rprimitive)
            )
...
def try_finally_entry_blocks(
    ...
    if ret_reg:
        builder.assign(
            ret_reg, builder.add(LoadErrorValue(builder.ret_types[-1], line)), line
        )
...
def try_finally_resolve_control(
    ...
    if ret_reg:
        ...
        if isinstance(ret_reg, AssignmentTargetAttr) and ret_reg.attr.startswith(
            TEMP_ATTR_NAME
        ):
...
def transform_try_finally_stmt_async(
    ...
    if ret_reg:
        ...
        builder.activate_block(return_block)
        builder.nonlocal_control[-1].gen_return(
            builder, builder.read(ret_reg, line), line
        )
...
def emit_yield_from_or_await(
    ...
    if isinstance(iter_reg.type, RInstance) and iter_reg.type.class_ir.has_method(
        helper_method
    ):
    ...
    def try_body() -> None:
        builder.assign(
            received_reg,
            emit_yield(builder, builder.read(to_yield_reg, line), line),
            line,
        )
    ...
    def else_body() -> None:
        ...
        _y = builder.call_c(
            send_op,
            [builder.read(iter_reg, line), builder.read(received_reg, line)],
            line,
        )
        ok, stop = BasicBlock(), BasicBlock()
