...
def generate_getattr_wrapper(
    ...
    with builder.enter_method(ir, name, object_rprimitive, internal=True):
        ...
        generic_getattr_result = builder.call_c(
            generic_getattr, [builder.self(), attr_arg], line
        )
...
def generate_setattr_wrapper(
    ...
    with builder.enter_method(ir, name, c_int_rprimitive, internal=True):
        ...
        delattr_override = delattr is not None and not delattr.fullname.startswith(
            "builtins."
        )
        if delattr_override:
            builder.gen_method_call(
                builder.self(), "__delattr__", [attr_arg], None, line
            )
        else:
        ...
        builder.gen_method_call(
            builder.self(), setattr.name, [attr_arg, value_arg], None, line
        )
        builder.add(Return(Integer(0, c_int_rprimitive), line))
...
def handle_ext_method(builder: IRBuilder, cdef: ClassDef, fdef: FuncDef) -> None:
    ...
    if fdef.name == "__getattr__":
        ...
        if (
            not setattr
            or not setattr.node
            or setattr.node.fullname.startswith("builtins.")
        ):
            builder.error(
...
def handle_non_ext_method(
    ...
    if name in class_ir.method_decls:
        func_ir, func_reg = gen_func_item(
            builder, fdef, name, sig, cdef, make_ext_method=True
        )
...
def generate_singledispatch_dispatch_function(
    ...
    builder.call_c(
        exact_dict_set_item_op, [dispatch_cache, arg_type, uncached_impl], line
    )
...
def maybe_insert_into_registry_dict(builder: IRBuilder, fitem: FuncDef) -> None:
    ...
    for singledispatch_func, types in to_register.items():
        ...
        for typ in types:
            ...
            builder.call_c(
                exact_dict_set_item_op, [registry, loaded_type, to_insert], line
            )
        dispatch_cache = builder.builder.get_attr(
            dispatch_func_obj, "dispatch_cache", dict_rprimitive, line
        )
