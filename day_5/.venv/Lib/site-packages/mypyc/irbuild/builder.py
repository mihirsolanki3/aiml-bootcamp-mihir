...
class IRBuilder:
        ...
        if self.is_native_module(module) and self.is_same_group_module(module):
            ...
            if group_name is not None:
                ...
                shared_lib_obj = self.primitive_op(
                    dict_get_item_op,
                    [mod_dict, self.load_str(shared_lib_mod_name, line)],
                    line,
                )
    ...
    def check_if_module_loaded(
        ...
        comparison = self.translate_is_op(
            first_load, self.none_object(line), "is not", line
        )
    ...
    def get_module(self, module: str, line: int) -> Value:
        ...
        return self.primitive_op(
            dict_get_item_op, [mod_dict, self.load_str(module, line)], line
        )
    ...
    def assign_if_bitmap_unset(
        ...
        b = self.add(
            ComparisonOp(o, Integer(0, bitmap_rprimitive), ComparisonOp.EQ, line)
        )
    ...
    def assign(
        ...
        if isinstance(target, Register):
            self.add(
                Assign(target, self.coerce_rvalue(rvalue_reg, target.type, line), line)
            )
        ...
        for i in range(len(target.items)):
            ...
            if is_list_rprimitive(rvalue.type):
                ...
                if not IS_FREE_THREADED:
                    ...
                    item_value = self.primitive_op(
                        list_get_item_int64_op, [rvalue, index], line
                    )
            elif is_tuple_rprimitive(rvalue.type):
                index = Integer(i, c_pyssize_t_rprimitive)
                item_value = self.call_c(
                    tuple_get_item_unsafe_op, [rvalue, index], line
                )
            else:
    ...
    def get_sequence_type_from_type(self, target_type: Type) -> RType:
        ...
        # This elif-blocks are needed for iterating over classes derived from NamedTuple.
            ...
            rtypes = list(
                dict.fromkeys(self.mapper.type_to_rtype(item) for item in items)
            )
            if len(rtypes) == 1:
                return rtypes.pop()
            else:
    ...
    def is_same_group_module(self, module: str) -> bool:
        ...
        """
        return self.mapper.group_map.get(module) == self.mapper.group_map.get(
            self.module_name
        )
    ...
    def enter_method(
        ...
        try:
            ...
            decl = FuncDecl(
                name, class_ir.name, self.module_name, sig, internal=internal
            )
        ...
        return self.add(
            Call(
                FuncDecl(
                    ...
                    self.module_name,
                    FuncSignature(
                        [RuntimeArg("type", object_rprimitive)], bool_rprimitive
                    ),
                ),
                [obj],
                obj.line,
