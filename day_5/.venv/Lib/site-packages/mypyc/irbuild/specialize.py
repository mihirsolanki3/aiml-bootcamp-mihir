...
DunderSpecializer = Callable[
    ["IRBuilder", Expression, list[Expression], Expression], Value | None
]
...
def apply_function_specialization(
    ...
    return _apply_specialization(
        builder, expr, callee, get_call_target_fullname(callee)
    )
...
def translate_len(builder: IRBuilder, expr: CallExpr, callee: RefExpr) -> Value | None:
    if len(expr.args) == 1 and expr.arg_kinds == [ARG_POS]:
        ...
        obj = builder.accept(
            arg,
            can_borrow=is_list_rprimitive(expr_rtype) or isinstance(expr_rtype, RVec),
        )
        if is_sequence_rprimitive(expr_rtype) or isinstance(expr_rtype, RTuple):
            return get_expr_length_value(
                builder, arg, obj, expr.line, use_pyssize_t=False
            )
        else:
...
def translate_sum_call(
    ...
    builder.assign(
        retval,
        builder.coerce(builder.accept(start_expr), target_type, expr.line),
        expr.line,
    )
    ...
    def gen_inner_stmts() -> None:
        ...
        builder.assign(
            retval,
            builder.binary_op(retval, call_expr, "+", call_expr.line),
            call_expr.line,
        )
        ...
        for i, desc in enumerate(descs):
            ...
            builder.add_bool_branch(
                builder.primitive_op(
                    cast(PrimitiveDescription, desc), [obj], expr.line
                ),
                pass_block,
                next_block,
            )
...
def translate_str_format(
    ...
    if isinstance(callee, MemberExpr):
        ...
        if isinstance(folded_callee, str) and expr.arg_kinds.count(ARG_POS) == len(
            expr.arg_kinds
        ):
...
def str_encode_fast_path(
    ...
    if encoding in ["u8", "utf", "utf8", "cp65001"]:
        return builder.call_c(
            str_encode_utf8_strict, [builder.accept(callee.expr)], expr.line
        )
    elif encoding in ["646", "ascii", "usascii"]:
        return builder.call_c(
            str_encode_ascii_strict, [builder.accept(callee.expr)], expr.line
        )
    elif encoding in ["iso88591", "8859", "cp819", "latin", "latin1", "l1"]:
        return builder.call_c(
            str_encode_latin1_strict, [builder.accept(callee.expr)], expr.line
        )
...
def translate_object_new(
    ...
    if len(expr.args) != 1:
        builder.error(
            f"{call} supported only with 1 argument, got {len(expr.args)}", expr.line
        )
        return None
...
def translate_object_setattr(
    builder: IRBuilder, expr: CallExpr, callee: RefExpr
) -> Value | None:
    is_super = isinstance(expr.callee, SuperExpr)
    is_object_callee = is_object(callee)
    if not (
        (is_super and len(expr.args) >= 2) or (is_object_callee and len(expr.args) >= 3)
    ):
        return None
...
def translate_getitem_with_bounds_check(
    ...
    range_check = builder.primitive_op(
        range_check_op, [obj, adjusted_index], ctx_expr.line
    )
    ...
    builder.add(
        RaiseStandardError(
            RaiseStandardError.INDEX_ERROR, "index out of range", ctx_expr.line
        )
    )
    builder.add(Unreachable())
    ...
    result = builder.primitive_op(
        get_item_unsafe_op, [obj, adjusted_index], ctx_expr.line
    )
