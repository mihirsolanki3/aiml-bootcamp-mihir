...
from mypyc.primitives.librt_vecs_ops import (
    vec_get_item_unsafe_borrow_op,
    vec_get_item_unsafe_op,
)
...
def vec_get_item_unsafe(
    ...
    *,
    ...
    return builder.primitive_op(
        desc, [base, index], line, type_args=[base.type.item_type]
    )
...
def vec_get_item_unsafe_lower(
    ...
    *,
    ...
    result = vec_load_mem_item(
        builder, item_addr, vtype.item_type, can_borrow=can_borrow
    )
...
def supports_vec_to_sequence(vec_type: RVec) -> bool:
    return (
        vec_api_by_item_type.get(vec_type.item_type) is not None
        or vec_type.depth() == 0
    )
