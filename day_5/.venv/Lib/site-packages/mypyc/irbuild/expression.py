...
def transform_name_expr(builder: IRBuilder, expr: NameExpr) -> Value:
    ...
    if expr.node is None:
        builder.add(
            RaiseStandardError(
                RaiseStandardError.NAME_ERROR,
                f'name "{expr.name}" is not defined',
                expr.line,
            )
        )
    ...
    if isinstance(callee, IndexExpr) and isinstance(callee.analyzed, TypeApplication):
        ...
        if (
            ...
            if len(expr.args) == 0 or (
                ...
                return vec_create(
                    builder.builder, vec_type, 0, expr.line, capacity=capacity
                )
...
def translate_super_method_call(
    ...
    for base in ir.mro[1:]:
        ...
        if callee.name == "__new__":
            result = translate_object_new(
                builder, expr, MemberExpr(callee.call, "__new__")
            )
            if result:
                return result
...
def translate_vec_create_from_iterable(
    ...
    *,
    ...
    if isinstance(arg, ListExpr):
        ...
        return vec_create_from_values(
            builder.builder, vec_type, items, line, capacity=capacity
        )
    if isinstance(arg, ListComprehension):
        return translate_vec_comprehension(
            builder, vec_type, arg.generator, capacity=capacity
        )
...
def vec_from_iterable(
    ...
    *,
    ...
    builder.assign(
        vec, vec_create(builder.builder, vec_type, 0, line, capacity=capacity), line
    )
    name = f"___tmp_{line}"
    var = Var(name)
...
def try_specialize_in_expr(
    ...
    if isinstance(rhs, (TupleExpr, ListExpr)):
        ...
        items = [
            builder.add(TupleGet(tuple_val, i))
            for i in range(len(tuple_val.type.types))
        ]
