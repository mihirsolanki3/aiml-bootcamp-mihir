...
def generate_class_reuse(
    ...
    context.declarations[name] = HeaderDeclaration(
        f"CPyThreadLocal {struct_name} *{name};"
    )
...
def generate_class(cl: ClassIR, module: str, emitter: Emitter) -> None:
    ...
    generate_methods_table(
        cl, methods_name, setup_name if generate_full else None, emitter
    )
    emit_line()
...
def emit_attr_defaults_func_call(
...
def generate_constructor_for_class(
    ...
    emit_setup_or_dunder_new_call(
        cl, setup_name, type_arg, not use_wrapper, new_args, emitter
    )
...
def generate_new_for_class(
    ...
    if (
        ...
        emitter.emit_lines(
            "if (ret == NULL) {", "    Py_DECREF(self);", "    return NULL;", "}"
        )
...
def generate_dealloc_for_class(
    ...
    if has_tp_finalize:
        ...
        emitter.emit_line(
            "int res = PyObject_CallFinalizerFromDealloc((PyObject *)self);"
        )
    ...
    if not cl.is_trait:
        for attr in cl.attributes:
            ...
            if attr in cl.final_attributes:
                ...
                emitter.emit_line(
                    f" (getter){getter_name(cl, attr, emitter.names)}, NULL,"
                )
            else:
                emitter.emit_line(
...
def generate_setter(cl: ClassIR, attr: str, rtype: RType, emitter: Emitter) -> None:
    ...
    if IS_FREE_THREADED and is_simple_refcounted_pointer(rtype):
        ...
        if deletable:
            emitter.emit_line("} else {")
            emitter.emit_line(
                f"CPy_SetAttrRef((PyObject **)&self->{attr_field}, NULL);"
            )
            emitter.emit_line("}")
        emitter.emit_line("return 0;")
        emitter.emit_line("}")
