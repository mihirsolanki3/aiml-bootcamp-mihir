...
class Emitter:
    ...
    def emit_box(
        ...
        if is_int_rprimitive(typ) or is_short_int_rprimitive(typ):
            ...
            if not typ.types:
                self.emit_line(
                    f"{declaration}{dest} = CPyTuple_LoadEmptyTupleConstant();"
                )
            else:
    ...
    def emit_gc_visit(self, target: str, rtype: RType) -> None:
        ...
        if not rtype.is_refcounted:
            ...
            prefix = VEC_MACRO_PREFIX[rtype._ctype]
            self.emit_line(
                f"if ({target}.items) {{ Py_VISIT({prefix}_BUF({target})); }}"
            )
    ...
    def _emit_traceback(
        ...
        if self.context.strict_traceback_checks:
            assert (
                traceback_entry[1] >= 0
            ), "Traceback cannot have a negative line number"
    ...
    def emit_cpyfunction_instance(
        ...
        has_self_arg = (
            "true" if fn.class_name and fn.decl.kind != FUNC_STATICMETHOD else "false"
        )
...
def stable_literal_repr(obj: object) -> str:
    ...
    if isinstance(obj, frozenset):
        if not obj:
            return "frozenset()"
        items = ", ".join(
            stable_literal_repr(item) for item in sorted(obj, key=literal_sort_key)
        )
