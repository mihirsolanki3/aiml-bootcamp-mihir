...
class FunctionEmitterVisitor(OpVisitor[None]):
    ...
    def emit_load_attr_take_ref(
        ...
        if use_get_attr_ref and cl.is_final_attr(op.attr):
            self.emitter.emit_line(
                f"{dest} = CPy_GetAttrRefFinal((PyObject **)&{attr_expr});"
            )
        elif use_get_attr_ref:
            self.emitter.emit_line(
                f"{dest} = CPy_GetAttrRef((PyObject **)&{attr_expr});"
            )
        else:
    ...
    def visit_set_attr(self, op: SetAttr) -> None:
        ...
        if op.is_propset:
            ...
            if op.is_init:
                ...
                # CPy_InitAttrRef).
                self.emitter.emit_line(
                    f"CPy_InitAttrRef((PyObject **)&{attr_expr}, {src});"
                )
            else:
                # Atomically swap in the new value and reclaim the old one.
                self.emitter.emit_line(
                    f"CPy_SetAttrRef((PyObject **)&{attr_expr}, {src});"
                )
            if op.error_kind == ERR_FALSE:
    ...
    def emit_method_call(
        ...
        if is_direct:
            ...
            self.emit_line(
                f"{dest}{self.emitter.native_function_call(method_decl)}({args});"
            )
        else:
            # Call using vtable.
    ...
    def visit_inc_ref(self, op: IncRef) -> None:
        if (
            ...
            and (
                ...
                or is_bool_or_bit_rprimitive(op.src.src.type)
            )
            and HAVE_IMMORTAL
        ):
    ...
    def visit_set_element(self, op: SetElement) -> None:
        ...
        if isinstance(op.src, Undef):
            ...
            self.emit_line(
                f"{dest} = ({self.ctype(src_type)}) {{ {', '.join(init_items)} }};"
            )
