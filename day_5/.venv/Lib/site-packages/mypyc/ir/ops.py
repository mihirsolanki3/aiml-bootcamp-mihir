...
class Box(RegisterOp):
    ...
    def __init__(self, src: Value, line: int = -1) -> None:
        ...
        if is_none_rprimitive(self.src.type) or is_bool_or_bit_rprimitive(
            self.src.type
        ):
...
class LoadMem(RegisterOp):
    ...
    def __init__(
        self, type: RType, src: Value, line: int = -1, *, borrow: bool = False
    ) -> None:
        super().__init__(line)
        self.type = type
...
class KeepAlive(RegisterOp):
    ...
    def __init__(
        self, src: list[Value], line: int = -1, *, steal: bool = False
    ) -> None:
        super().__init__(line)
        assert src
        self.src = src
