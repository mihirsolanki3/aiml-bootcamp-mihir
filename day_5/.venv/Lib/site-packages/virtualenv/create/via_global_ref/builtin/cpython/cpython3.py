...
class CPython3Posix(CPythonPosix, CPython3):
    ...
    def _shared_libpython(cls, interpreter: PythonInfo) -> Path | None:
        ...
        if (
            not isinstance(
                instsoname := interpreter.sysconfig_vars.get("INSTSONAME"), str
            )
            or not instsoname
        ):
            return None
        if (
            not isinstance(libdir := interpreter.sysconfig_vars.get("LIBDIR"), str)
            or not libdir
        ):
            return None
        ...
        if interpreter.version_info >= (3, 13):
            ...
            for ref in sources:
                if ref.base == "python.exe":
                    launcher_path = host.with_name(
                        f"venvlauncher{t_suffix}{d_suffix}.exe"
                    )
                    ...
                    w_launcher_path = ref.src.with_name(
                        f"venvwlauncher{t_suffix}{d_suffix}.exe"
                    )
