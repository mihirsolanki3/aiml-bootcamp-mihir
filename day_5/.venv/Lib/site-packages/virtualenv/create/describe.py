...
class Describe:
    ...
    def _calc_config_vars(self, to: Path) -> dict[str, Any]:
        ...
        ) is None:  # no prefix means no value can live under it
            return dict(sys_vars)
        return {
            k: (to if isinstance(v, str) and v.startswith(prefix) else v)
    ...
    ) -> bool:  # ruff:ignore[unused-class-method-argument]
